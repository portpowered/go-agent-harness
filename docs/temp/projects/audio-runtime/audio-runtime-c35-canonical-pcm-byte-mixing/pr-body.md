...
- Evidence candidate at the verifier run: `201a159dcd30c1a029321f2ab4e27b73cd944c61`
- The verifier records the candidate as an evidence-only descendant of the tested source and proves executable-input equality across all 12 hashed inputs, including `consumer/go.sum`.
...
- The verifier build-input manifest records `consumer/go.sum` as `0eb084ae04738236d00d96d849c231482df0549e2456fcf8637f4bceb96c6857`.

## Validation

- Full owned verifier `--mode all` from clean source `e695f03e` against candidate `201a159d` returned `ACCEPTED` in `29.610367s`; normal/race mixer and room tests, targeted vet, architecture-size-check, diff-check, consumer controls, and public replay controls all passed.
- The changed-path allowlist reports only owned production/test paths and owned evidence; `progress.txt` is absent and outside-path violations are empty.
...
The current-main merge and evidence cleanup are confined to the admitted C35 branch and owned evidence directory. The final handoff commit is an evidence-only descendant of verifier candidate `201a159d`; no executable input changed after the equality-tested candidate. This handoff claims no script-CI result, independent review, guarded merge, post-merge vertical acceptance, physical/acoustic proof, or project completion. Submit the same PR to the script CI gate; do not infer CI status from this local handoff.
