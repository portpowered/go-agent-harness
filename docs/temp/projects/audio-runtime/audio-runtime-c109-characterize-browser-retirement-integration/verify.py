...
RUNNER_RELATIVE = OWNED_PREFIX + "run_public_checks.py"
FIXTURE_RELATIVE = OWNED_PREFIX + "fixtures/negative-cases.json"
...
def validate_public_report(data: dict[str, Any], expected_case: str | None = None) -> None:
    ...
    require(runner.get("runner_path") == RUNNER_RELATIVE, "public runner path binding is missing")
    require(re.fullmatch(r"[0-9a-f]{64}", runner.get("runner_sha256", "")), "public runner source hash is missing")
    require(runner["runner_sha256"] == sha256_file(root / RUNNER_RELATIVE), "public report was generated by a stale runner")
    require(runner.get("fixture_path") == FIXTURE_RELATIVE, "public fixture path binding is missing")
    require(re.fullmatch(r"[0-9a-f]{64}", runner.get("fixture_sha256", "")), "public fixture hash is missing")
    require(runner["fixture_sha256"] == sha256_file(root / FIXTURE_RELATIVE), "public report was generated from a stale negative fixture")
    ...
    observed = integrity.get("observed_before_cleanup", {})
    require(observed.get("head") == synthetic.get("tree_binding", {}).get("head") and observed.get("tree") == synthetic.get("tree") and observed.get("status") == [], "public synthetic tree identity/cleanliness is not preserved")
    ...
    for item in checks:
        if "go test" in item.get("command", ""):
            ...
            require(discovery.get("required") is True and discovery.get("valid") is True and discovery.get("tests_discovered", 0) > 0, f"zero-test discovery accepted: {item.get('label')}")
        if "test-session-ci-regressions.sh" in item.get("command", ""):
            discovery = item.get("test_discovery", {})
            require(discovery.get("required") is True and discovery.get("valid") is True and discovery.get("tests_discovered", 0) > 0, f"zero-test accumulated regression accepted: {item.get('label')}")
...
def run_caller_tree_mutation_control() -> dict[str, Any]:
    from run_public_checks import bounded, cleanup_tree, create_required_tree, synthetic_tree_binding, tree_identity_unchanged

    temporary_root, worktree, expected_tree = create_required_tree()
    marker = worktree / ".c109-caller-tree-mutation-control"
    detected = False
    check: dict[str, Any] = {}
    cleanup: dict[str, Any] = {}
    try:
        expected = synthetic_tree_binding(worktree, expected_tree)
        check = bounded(
            [sys.executable, "-c", f"from pathlib import Path; Path({str(marker)!r}).write_text('intentional control', encoding='utf-8')"],
            worktree,
            10,
            label="caller-owned tree mutation control",
        )
        require(check.get("status") == "passed", "caller-tree mutation control child did not complete")
        detected = not tree_identity_unchanged(worktree, expected)
        require(detected, "caller-tree mutation guard accepted a dirty caller-owned tree")
    finally:
        marker.unlink(missing_ok=True)
        cleanup = cleanup_tree(temporary_root, worktree)
    require(cleanup.get("status") == "passed", "caller-tree mutation control cleanup failed")
    return {
        "status": "rejected-as-required" if detected else "failed",
        "child_status": check.get("status"),
...
def main() -> int:
    ...
    result: dict[str, Any] = {"mode": args.mode, "checks": [], "source_bindings": source_bindings(root)}
