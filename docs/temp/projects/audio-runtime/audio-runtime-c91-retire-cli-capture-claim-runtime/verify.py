...
EVIDENCE_PREFIXES = (
    ...
    "docs/temp/projects/audio-runtime/audio-runtime-c160-recover-c151-capture-claim-ci-handoff/",
    "docs/temp/projects/audio-runtime/audio-runtime-c164-recover-c160-capture-claim-policy-handoff/",
    "docs/temp/projects/audio-runtime/audio-runtime-c171-recover-c164-capture-claim-review-handoff/",
)
...
CAPTURECLAIM_BASELINE = (
    "docs/architecture/baselines/github.com/portpowered/go-agent-harness/"
    "agent-cli/internal/services/internal/agentruntime/session_capture_claim.go.json"
)
CAPTURECLAIM_WIRE_ENTRY = "go-agent-runtime/services/captureclaim/wire"
CAPTURECLAIM_POLICY_ENTRY = {
    "module": "go-agent-runtime",
    "pattern": "services/captureclaim/wire/wire_gen.go",
    "generator": "wire",
    "header": "// Code generated by Wire. DO NOT EDIT.",
}
CAPTURECLAIM_RETIRED_SYMBOL = "acquireSessionRecordingClaim"
...
def check_scope() -> None:
    ...
    check_captureclaim_shared_entries(changed)


def check_captureclaim_shared_entries(changed: set[str]) -> None:
    if WIRE_REGISTRY in changed:
        baseline_lines = git("show", f"{SCOPE_BASE}:{WIRE_REGISTRY}").splitlines()
        candidate_lines = (ROOT / WIRE_REGISTRY).read_text(encoding="utf-8").splitlines()
        require(
            candidate_lines == baseline_lines + [CAPTURECLAIM_WIRE_ENTRY],
            "captureclaim Wire registry change is not exactly one appended entry",
        )

    if ARCHITECTURE_POLICY in changed:
        baseline_policy = json.loads(git("show", f"{SCOPE_BASE}:{ARCHITECTURE_POLICY}"))
        candidate_policy = json.loads((ROOT / ARCHITECTURE_POLICY).read_text(encoding="utf-8"))
        expected_policy = dict(baseline_policy)
        expected_policy["generated_files"] = baseline_policy["generated_files"] + [CAPTURECLAIM_POLICY_ENTRY]
        require(candidate_policy == expected_policy, "captureclaim architecture policy change is not the exact generated-file entry")

    if CAPTURECLAIM_BASELINE in changed:
        baseline = json.loads(git("show", f"{SCOPE_BASE}:{CAPTURECLAIM_BASELINE}"))
        candidate = json.loads((ROOT / CAPTURECLAIM_BASELINE).read_text(encoding="utf-8"))
        expected = dict(baseline)
        expected["entries"] = [
            entry
            for entry in baseline["entries"]
            if entry.get("symbol") != CAPTURECLAIM_RETIRED_SYMBOL
        ]
        require(candidate == expected, "captureclaim baseline change is not limited to the retired symbol entries")
