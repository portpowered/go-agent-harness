...
- `make architecture-size-check` now reports only the nine demonstrated stale C103 entries in the C79-owned architecture baseline, and `make wire-check` reports only the unregistered C103 generated graph in the C79-owned Wire registry. No shared file was edited.
