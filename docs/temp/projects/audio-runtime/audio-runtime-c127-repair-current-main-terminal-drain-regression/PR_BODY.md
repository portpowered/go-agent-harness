...
Repair the current-main live terminal-drain race by claiming provider RTC media immediately after `ConnectSession()` and before `Receive()`. The existing continuous-audio option is preserved and the later media attachment remains idempotent. Source checkpoint: `abad266fb092b78ca141afc050b86018f1aba216`; the candidate also contains freshly fetched `origin/main` `bd6a1289218d1bef1a3af36e64e9d4496062416f`.
...
- The prior SCRIPT CI rejection additionally found the 772-line causal test and 26/28 complexity scores; the test is now isolated in `terminal_drain_test.go` (218 lines) with helper-level assertions, while `service_test.go` is 600 lines. The same CI coverage job failed the out-of-lease browser interruption test; it remains preserved for its owner and is not claimed as a C127 result.
