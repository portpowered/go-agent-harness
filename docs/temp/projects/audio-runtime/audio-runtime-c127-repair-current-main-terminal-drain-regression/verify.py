...
def verify_public(report: dict, head: str, origin_main: str) -> None:
    ...
    for case_name in ("software-device-tool-drain", "credential-free-audio-tool-replay"):
        ...
        require(isinstance(command, list), f"public command missing: {case_name}")
        effects = case.get("effects")
        require(isinstance(effects, dict), f"public effects missing: {case_name}")
        if case_name == "software-device-tool-drain":
            require("--audio-device-server" in command, "software-device case did not use the public device-server route")
            require("--audio-out-device=" in command, "software-device case did not select the simulated output device")
            device = case.get("software_device")
            require(isinstance(device, dict), "software-device evidence is missing")
            require(device.get("exact_output_pcm_contained") is True, "software-device PCM evidence is missing")
            device_wav = device.get("device_output_wav")
            require(isinstance(device_wav, dict), "software-device output WAV evidence is missing")
            require(device_wav.get("sample_rate") == 16000, "software-device output WAV rate changed")
            require(device_wav.get("channels") == 1 and device_wav.get("sample_width_bytes") == 2, "software-device output WAV format changed")
            require(isinstance(device_wav.get("frames"), int) and device_wav["frames"] > 0, "software-device output WAV is empty")
            require(device_wav.get("pcm_bytes") == device_wav["frames"] * 2, "software-device output WAV PCM accounting is inconsistent")
            require(device_wav.get("wav_bytes") == effects.get("audio_wav_bytes"), "software-device output WAV size is inconsistent")
            require(device_wav.get("wav_sha256") == effects.get("audio_wav_sha256"), "software-device output WAV hash is inconsistent")
            snapshot = device.get("snapshot")
            require(isinstance(snapshot, dict), "software-device snapshot is missing")
            playback = snapshot.get("playback")
            rendered = snapshot.get("rendered_samples")
            require(isinstance(playback, dict) and isinstance(rendered, list), "software-device snapshot is incomplete")
            for field in ("QueuedSamples", "DroppedSamples", "OverflowEvents", "DiscardedSamples", "DiscardEvents"):
                require(playback.get(field) == 0, f"software-device {field} is nonzero")
            require(playback.get("RenderedSamples") == len(rendered), "software-device rendered accounting is inconsistent")
            cleanup_record = device.get("cleanup")
            require(isinstance(cleanup_record, dict), "software-device cleanup is missing")
            cleanup = cleanup_record.get("cleanup")
            require(isinstance(cleanup, dict), "software-device cleanup details are missing")
            require(cleanup.get("parent_reaped") is True and cleanup.get("group_alive_after") is False, "software-device cleanup is incomplete")
        else:
            require("--audio-device-server" not in command, "credential-free replay unexpectedly used the device-server route")
            require("--audio-out-device=" not in command, "credential-free replay unexpectedly selected a device")
