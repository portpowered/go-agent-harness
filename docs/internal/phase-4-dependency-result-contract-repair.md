...
- `go-agent-loop/pkg/messages.Session.Send` returns `bool`; the current public
  ...
  buffer.
- `go-agent-loop/pkg/messages.TypedBuffer.ReadBlockingContext` returns
  `(T, bool)`; the current public declaration does not distinguish cancellation
  from another closed `done` signal.
...
| --- | --- | --- | --- |
| `P4-CTX-*` | `uncertain` | The current public APIs show broad context-first cancellation coverage on `AgenticLoop`, `Inferencer`, `ToolExecutor`, `SessionInferencer`, `Session.Send`, and `TypedBuffer.ReadBlockingContext`. The exact reconciled `P4-CTX-*` row list is missing because `phase-4-api-audit-reconciliation` is not present in this checkout. | Import or link the reconciled audit row list, then close only the rows whose blocking public calls already return inspectable cancellation errors or have additive repairs. At minimum, future story work should verify cancellation outcomes for buffer/session/result helpers that still return only `bool` or zero values. |
...
| `P4-API-01` | `uncertain` | `AgenticLoop`, `Inferencer`, `ToolExecutor`, and `SessionInferencer` use context-first signatures, so caller-owned cancellation is visible on major blocking surfaces. Ambiguous helper outcomes remain on `TypedBuffer.ReadBlockingContext`, `Session.Send`, `Stream.HasNext`, and final text helpers. | Define the exact `P4-API-01` row from the reconciled audit and close only the covered declarations. Later stories should add cancellation/result tests for repaired ambiguous helpers. |
...
- Add typed buffer/session outcome APIs without removing existing `bool`
  methods.
