...
| `P4-TT-RECON-03` | Public typed errors and direct stream terminal fields | Returned representative gateway/provider failures preserve caller-actionable classes through `errors.Is`/`errors.As`, and direct gateway stream `ERROR` payloads expose serialized `classification`, `terminal_reason`, `terminal_provenance`, and `output_state` while retaining readable text. | `go-llm-gateway/pkg/gateway/errors.go`, `go-llm-gateway/pkg/providers/errors.go`, `go-llm-gateway/pkg/gateway/gateway_test.go`, `go-llm-gateway/pkg/providers/errors_test.go`, `docs/architecture/stream-terminal-contract.md`, `docs/internal/phase-4-typed-terminal-authoritative-reconciliation.md` |
| `P4-TT-RECON-04` | Replay, cancellation, and partial-output terminal evidence | Replay divergence, replay incomplete, cancellation, and partial output remain distinguishable through public replay outcomes, typed errors, loop final-text results, stream outcomes, and terminal event metadata. | `go-llm-gateway/pkg/testing/session_replay.go`, `go-llm-gateway/pkg/testing/session_replay_test.go`, `go-agent-loop/pkg/agentloop/execute_result.go`, `go-agent-loop/pkg/agentloop/execute_result_test.go`, `docs/architecture/stream-terminal-contract.md`, `docs/internal/phase-4-typed-terminal-authoritative-reconciliation.md` |
| `P4-TT-RECON-05` | CLI and session terminal surfaces | Representative OpenAI and Grok session close and terminal error events expose public terminal metadata, and `agent session` replay output maps those fields to stable key/value status lines while preserving legacy readable output. | `go-llm-gateway/pkg/providers/openai/session.go`, `go-llm-gateway/pkg/providers/openai/session_test.go`, `go-llm-gateway/pkg/providers/grok/events.go`, `go-llm-gateway/pkg/providers/grok/session_test.go`, `agent-cli/internal/services/session.go`, `agent-cli/internal/services/session_test.go`, `agent-cli/test/integration/session_command_test.go`, `docs/architecture/stream-terminal-contract.md`, `docs/internal/phase-4-typed-terminal-authoritative-reconciliation.md` |
| `P4-TT-RECON-06` | Docs and audit alignment | Public docs and internal audit evidence describe the landed typed-terminal taxonomy, identify which terminal states return as Go errors versus in-band stream/session/CLI events, map representative evidence to `P4-API-02`, `P4-API-03`, `P4-API-05`, and keep whole-Phase-4 `P4-GATE-01` open. | `docs/architecture/stream-terminal-contract.md`, `go-llm-gateway/README.md`, `docs/architecture/contract-gap-audit.md`, `docs/internal/phase-4-typed-terminal-authoritative-reconciliation.md`, `docs/internal/checklist.md` |
| `P4-TT-RECON-07` | Credential-free reviewer validation | Reviewer-runnable commands and a validation brief prove branch disposition, typed errors, direct stream terminal fields, replay, cancellation, partial output, CLI/session terminal surfaces, docs alignment, and quality gates without live provider credentials while explicitly keeping whole-Phase-4 `P4-GATE-01` open. | `docs/internal/phase-4-typed-terminal-authoritative-reconciliation.md`, `docs/internal/checklist.md`, root `Makefile`, typed-terminal gateway/provider/loop/replay/session/CLI tests |
...
This checklist is the authoritative Phase 4 inventory for public API contract
hardening. The Phase 4 repair validator must cite these item IDs directly when
it records `pass`, `fail`, or `uncertain` evidence.

| Item ID | Area | Required outcome | Primary evidence surfaces |
| --- | --- | --- | --- |
| `P4-API-01` | Context ownership | Public blocking calls expose caller-controlled cancellation and timeout behavior anywhere they wait, perform external work, relay streams, replay fixtures, or flush recordings. | `agent-cli/internal/services/*`, `agent-cli/internal/agent/*`, `go-llm-gateway/pkg/gateway/*`, `go-llm-gateway/pkg/testing/*`, provider packages, deterministic cancellation tests, public docs |
| `P4-API-02` | Typed errors | Public gateway, provider, CLI, replay, and validation boundaries expose typed caller-actionable errors that support `errors.Is` or `errors.As` instead of requiring string parsing. | `go-llm-gateway/pkg/gateway/*`, `go-llm-gateway/pkg/providers/*`, `go-llm-gateway/pkg/testing/*`, `agent-cli/internal/output/*`, docs, deterministic error tests |
| `P4-API-03` | Result contracts | Public result, buffer, session, and stream APIs expose unambiguous outcomes for empty success, cancellation, partial success, closed or drained state, and terminal failure. | `go-agent-loop/pkg/messages/*`, `go-agent-loop/pkg/agentloop/*`, `go-llm-gateway/pkg/gateway/*`, `go-llm-gateway/pkg/testing/*`, CLI output contracts, docs, deterministic result tests |
| `P4-API-04` | Provider capability discovery | Public APIs expose provider capabilities for tools, streaming, sessions, audio, image input, video output, reasoning, prompt caching, and provider-specific configuration with supported, unsupported, and unknown states. | `go-llm-gateway/pkg/providers/*`, `go-llm-gateway/pkg/gateway/*`, provider docs, examples where present, deterministic capability tests |
| `P4-API-05` | Stream semantics | Public streaming APIs preserve terminal events, cancellation, replay mismatch, partial output, and typed error details through observable stream events or documented result surfaces. | `go-llm-gateway/pkg/gateway/*`, `go-llm-gateway/pkg/inference/*`, provider stream implementations, `go-llm-gateway/pkg/testing/*`, stream docs, deterministic stream tests |
| `P4-API-06` | Local unsupported-feature validation | Unsupported stateless and session request features fail locally before provider execution with inspectable errors identifying feature, provider, requested mode, and capability state. | `go-llm-gateway/pkg/gateway/*`, `go-llm-gateway/pkg/providers/*`, `agent-cli/internal/agent/*`, `agent-cli/internal/services/*`, docs, deterministic validation tests |
| `P4-API-07` | Dependency ownership and side effects | Constructors, provider runtime seams, prompt resolution, filesystem, environment, process, transport, network, and time dependencies are caller-owned, injected, side-effect free, or explicitly documented as open work. | `docs/architecture/dependencies.md`, `docs/architecture/contract-gap-audit.md`, `agent-cli/internal/*`, `go-llm-gateway/pkg/providers/*`, `go-llm-gateway/pkg/testing/*`, deterministic ownership tests |
| `P4-GATE-01` | Public API hardening gate | The Phase 4 public API contract hardening baseline is review-ready only when docs, examples where present, audit rows, public APIs, deterministic tests, and reviewer-runnable commands describe the same current contract. | `docs/internal/phase-4-api-contract-repair-validator.md`, `docs/internal/checklist.md`, `docs/architecture/contract-gap-audit.md`, public docs, examples where present, root quality targets |
