...
| --- | --- | --- | --- |
| `P3-CORE-01` | Authoritative shared contract boundary | The repository names `go-agent-loop/pkg/messages` as the authoritative shared contract boundary for cross-library message, stream, tool, token-usage, inference, and session contracts, with reviewer-citable rationale for keeping that boundary in the loop module during this phase. | `go-agent-loop/pkg/messages/doc.go`, `docs/architecture/dependencies.md`, `docs/architecture/contract-gap-audit.md` |
| `P3-CORE-02` | Gateway compatibility layer and bridge evidence | Gateway-facing shared-message surfaces are documented as compatibility aliases over loop-owned contracts, public adapter packages are described as bridges into that boundary, and automated dependency evidence proves the intended one-way import direction without reverse loop-to-gateway imports. | `go-llm-gateway/pkg/models/doc.go`, `go-llm-gateway/pkg/models/message.go`, `go-llm-gateway/pkg/inference/doc.go`, `go-llm-gateway/pkg/gateway/session_gateway.go`, `docs/architecture/dependencies.md`, `docs/architecture/contract-gap-audit.md`, `go-agent-loop/test/architecture/dependency_direction_test.go` |
