...
### Replay and composition checkpoints, 2026-09-05

`1783b3f2` retains finite audio dispatch and provider-close boundaries; `8334accf` composes physical playback and file recording inside the device service. The recording tap has bounded admission, preserves admitted samples across caller cancellation, and reports recording failures at finalization without stopping healthy playback. Exact remote-device PCM and scoped device architecture checks pass. These observations are post-conversion playback admission; they do not establish physical speaker callback timing.

`b7e39b2c` preserves Grok transport failures and delegates payload mismatch diagnostics to the strict replay transport. `0ec1c05f` presents the joined replay result and recorded initial tool advertisement without changing the execution allowlist; `14f1063a` retains recorded human-readable close notices. `ef9f9fb5` verifies exact scheduled PCM across bounded provider packets, with one commit/response boundary per turn. The bare-replay and permitted/rejected tool checks pass together. The independent embedding race suite passes on the subsequent working tree. A broader session subset completed with 40 passes and three failures: two close-notice regressions have since passed focused checks, and the remaining captured-handshake fixture requires explicit model-catalog injection. This is not a complete CLI acceptance result.

The model catalog is being moved behind provider-owned contracts and Wire, including legacy callers and test fixtures. Its checkpoint remains pending full review of dependency admission and measured shape changes. Raw provider recording still needs bounded production storage: the implementation under review reserves outbound ordering before transport writes, settles accepted/discarded events, and streams an integrity-compatible envelope from a spool. Production must never silently select the memory-backed fixture recorder, and retained unsettled reservations must count against the memory budget. The new streaming API must not expose a sealed empty snapshot in place of the spooled records. Custom-tool fixtures are moving to whole-service injection so definitions, execution, and lifecycle remain paired. Final aggregate tests, remaining legacy/diagnostic extraction, physical evidence, long-session resource bounds, and release verification are still required before merge.

### Checkpoint and rollback discipline

Commit each coherent reviewed slice with its matching contracts, generated injectors and regression tests. Record the checks actually run and any unresolved integration failures; a checkpoint does not imply release readiness. Stage shared files by owned changes so unrelated work in progress does not enter a checkpoint accidentally. Preserve these commits without rewriting history, allowing a regression to be traced or a completed slice to be reverted independently. Before merge, run the aggregate release checks against one stable revision rather than combining results from different intermediate trees.
