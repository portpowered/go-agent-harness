...
1. **Stabilize the shared implementation.** Complete the current CLI host extraction, retain composition/preflight assertions through a valid public replay fixture, and obtain a fresh aggregate test and analyzer result. The latest broad `make test` attempt failed the composition fixture and timed out in the CLI test command. A bounded integration diagnostic then exposed persistent-input end-of-turn handling, replay/tool admission, terminal classification and injected-provider capture parity failures, plus an in-band provider close/join cycle. These are migration regressions to resolve through the intended public owners; focused green tests do not supersede them.
