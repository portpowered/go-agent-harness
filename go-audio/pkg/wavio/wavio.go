...
func SupportedSampleRates() []int { return []int{Rate16kHz, Rate24kHz, Rate48kHz} }
...
func Write(w io.Writer, sampleRate int, samples []int16) error {
	...
	if !isSupportedRate(sampleRate) {
		return &UnsupportedError{Property: "sample rate", Observed: sampleRate, Supported: "16000, 24000 or 48000 Hz"}
	}
	if len(samples) == 0 {
	...
	var encoded [32 * 1024]byte
	for len(samples) > 0 {
		n := min(len(samples), len(encoded)/2)
		if err := codec.EncodePCM16Into(encoded[:n*2], samples[:n]); err != nil {
			return err
		}
		if err := writeAll(w, encoded[:n*2]); err != nil {
			return err
		}
		samples = samples[n:]
	}
...
func validatePCM16Format(payload [16]byte) (waveFormat, error) {
	...
	if rate == 0 || uint64(rate) > uint64(maxUint32)/pcm16BlockAlign {
		return waveFormat{}, &UnsupportedError{Property: "sample rate", Observed: rate, Supported: "positive PCM16 rate with a representable RIFF byte rate"}
	}
...
func isSupportedRate(rate int) bool {
	return rate == Rate16kHz || rate == Rate24kHz || rate == Rate48kHz
}

// ValidateSampleRate checks the rates supported by the stream resampler.
// Container Inspect and StreamWriter accept other valid PCM16 sample rates.
func ValidateSampleRate(rate int) error {
	if !isSupportedRate(rate) {
		return &UnsupportedError{Property: "sample rate", Observed: rate, Supported: "16000, 24000 or 48000 Hz"}
	}
	return nil
}
func validateFormat(payload [16]byte) (waveFormat, error) {
	...
	if err := ValidateSampleRate(int(format.sampleRate)); err != nil {
		return waveFormat{}, err
	}
	return format, nil
}
