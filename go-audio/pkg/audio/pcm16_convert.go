...
func ResamplePCM16(samples []int16, inputRate, outputRate int) ([]int16, error) {
	...
	for outputIndex := range converted {
		...
		if !isFinitePCM16Position(position) {
			return nil, fmt.Errorf("%w: nonfinite source position at output sample %d", ErrPCM16ConversionSize, outputIndex)
		}
...
func ConvertPCM16Bytes(pcm []byte, sourceChannels, sourceRate, targetChannels, targetRate int) ([]byte, error) {
	...
	if sourceChannels > maximumPCM16Int()/2 {
		return nil, fmt.Errorf("%w: source frame size for %d channels", ErrPCM16ConversionSize, sourceChannels)
	}
	...
	for outputFrame := 0; outputFrame < outputFrames; outputFrame++ {
		...
		if !isFinitePCM16Position(position) {
			return nil, fmt.Errorf("%w: nonfinite source position at output frame %d", ErrPCM16ConversionSize, outputFrame)
		}
		lower := int(position)
