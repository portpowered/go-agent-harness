...
func (m *sessionInboundMedia) activatePlaybackLocked(response PlaybackResponse) {
	...
	// The controller owns device-consumption accounting after dequeue. Retain
	// provider totals only for ingress and queued responses; old responses must
	// not accumulate for the lifetime of a long conversation.
	if previous != m.response {
...
func (m *sessionInboundMedia) startResponse(response PlaybackResponse) {
	...
	if m.responseSamples[m.response] == 0 {
...
func (m *sessionInboundMedia) interrupt() (PlaybackInterruption, bool) {
	...
	if m.sampleRate > 0 {
		...
		if interruption.PlaybackResponse == response {
			availableSamples, known = responseSamples, true
		}
		// A response already handed to the device can have been retired from
		// this ingress ledger. Its precise controller receipt is authoritative.
		if known {
...
func (m *sessionInboundMedia) fail(err error) {
	...
	if m.terminal == nil && !m.closed {
		if !m.appendResponseBoundaryLocked(false) {
			// A partial response tail must never disappear silently when the
			// bounded frame queue is already full. The caller cannot receive a
			// separate error from FailInbound, so retain the provider cause and
			// classify the dropped tail on the terminal read instead.
			err = errors.Join(err, ErrSessionMediaInboundBacklog)
		}
		m.activateQueuedPlaybackLocked()
		m.terminal = err
	}
...
func (m *sessionInboundMedia) appendResponseBoundaryLocked(includeEmpty bool) bool {
	if len(m.frames) >= sessionMediaMaxQueuedFrames {
		if len(m.pending) > 0 || (includeEmpty && !m.padPartial) {
			m.pending = nil
			return false
		}
		return true
