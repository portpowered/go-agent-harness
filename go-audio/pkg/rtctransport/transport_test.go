...
func TestInboundTransportOrdersPacketsAndUsesOnePLCFrameForOneGap(t *testing.T) {
	...
	track, err := NewInboundTrack(source, decoder, InboundTrackConfig{})
	if err != nil {
	...
	if _, readErr := track.ReadFrame(ctx); !errors.Is(readErr, ErrInboundTrackSource) {
...
func TestInboundTransportRejectsNonV2AndImpossibleProgress(t *testing.T) {
	tests := []struct {
		...
		{name: "non-v2", packet: testPacket(1, 1000, 1, 111, 1), want: ErrInvalidInboundRTPPacket},
		{name: "timestamp jump", packet: testPacket(2, 3000, 1, 111, 2), want: ErrImpossibleRTPProgress},
		{name: "ssrc change", packet: testPacket(2, 1960, 2, 111, 2), want: ErrInvalidInboundRTPPacket},
		{name: "payload change", packet: testPacket(2, 1960, 1, 112, 2), want: ErrInvalidInboundRTPPacket},
	}
...
func testInboundRejection(t *testing.T, name string, packet *rtp.Packet, want error) {
	...
	track, err := NewInboundTrack(
		&testPacketSource{packets: packets}, &testDecoder{}, InboundTrackConfig{},
	)
	if err != nil {
		t.Fatalf("NewInboundTrack() error = %v", err)
	}
	...
	for {
		...
		var typedErr *InboundTrackError
...
func TestOutboundTransportCommitsTimelineOnlyAfterSuccessfulWrite(t *testing.T) {
	...
	track, err := NewOutboundTrack(OutboundTrackConfig{
		SourceRate: 48000,
		Encoder: OpusEncoderFunc(func(context.Context, []int16) ([]byte, error) {
		...
		Pacer:                 PacerFunc(func(context.Context, uint64) error { return nil }),
...
func TestOutboundTransportRejectsPartialFramesAndBoundsConcurrentWriters(t *testing.T) {
	...
	track, err := NewOutboundTrack(OutboundTrackConfig{
		...
		Pacer: PacerFunc(func(ctx context.Context, _ uint64) error {
	...
	frame := sharedaudio.PCMFrame{Samples: make([]int16, 960)}
	if writeErr := track.WriteFrame(context.Background(), sharedaudio.PCMFrame{Samples: make([]int16, 959)}); !errors.Is(writeErr, ErrOutboundFrameSize) {
	...
	if writeErr := track.WriteFrame(context.Background(), frame); !errors.Is(writeErr, ErrOutboundQueueOverflow) {
		t.Fatalf("saturated WriteFrame() error = %v, want queue overflow identity", writeErr)
	}
	if closeErr := track.Close(); closeErr != nil {
		t.Fatalf("Close() error = %v", closeErr)
	}
	if writeErr := <-firstDone; !errors.Is(writeErr, ErrOutboundClosed) {
...
func TestTransportConstructionRejectsInvalidDependencies(t *testing.T) {
	if _, err := NewInboundTrack(nil, &testDecoder{}, InboundTrackConfig{}); !errors.Is(err, ErrNilInboundRTPTrack) {
		t.Fatalf("nil inbound source error = %v", err)
	}
	if _, err := NewOutboundTrack(OutboundTrackConfig{}); !errors.Is(err, ErrOutboundNilEncoder) {
...
func TestInboundTerminalErrorDoesNotBlockWhenFrameQueueIsFull(t *testing.T) {
	...
	track, err := NewInboundTrack(source, &testDecoder{}, InboundTrackConfig{
		FrameDuration: 20 * time.Millisecond,
	...
	if !errors.Is(terminalErr, sourceErr) || !errors.Is(terminalErr, ErrInboundTrackSource) {
...
func TestInboundTerminalFailureClosesBlockedPacketSource(t *testing.T) {
	...
	defer cancel()
	_, terminalErr := track.ReadFrame(ctx)
	assertInboundKind(t, terminalErr, ErrInvalidInboundRTPPacket)
}
...
func TestOutboundConstructionRejectsTypedNilDependencies(t *testing.T) {
	validWriter := RTPWriterFunc(func(context.Context, *rtp.Packet) error { return nil })
	validEncoder := OpusEncoderFunc(func(context.Context, []int16) ([]byte, error) { return []byte{1}, nil })
	validPacer := PacerFunc(func(context.Context, uint64) error { return nil })

	var encoder *typedNilEncoder
	if _, err := NewOutboundTrack(OutboundTrackConfig{
		SourceRate: 48000, Encoder: encoder, Writer: validWriter, Pacer: validPacer,
	}); !errors.Is(err, ErrOutboundNilEncoder) {
	...
	}); !errors.Is(err, ErrOutboundNilWriter) {
	...
	}); !errors.Is(err, ErrOutboundNilPacer) {
