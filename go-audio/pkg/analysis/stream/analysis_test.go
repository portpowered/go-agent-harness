...
func TestAnalyzePCM16MeasuresFramesWithoutPaddingTheTail(t *testing.T) {
	...
	result, err := stream.AnalyzePCM16(stream.PCM16Input{
		StreamID:   "levels",
		SampleRate: 1000,
		Samples:    samples,
	}, stream.DefaultAnalysisConfig())
	if err != nil {
...
type syntheticDefectCase struct {
	name             string
	mutate           func(*stream.PCM16Input)
	wantProperties   []string
	wantNoProperties []string
}

func syntheticDefectCases() []syntheticDefectCase {
	return []syntheticDefectCase{
		{
			name: "clean",
			wantNoProperties: []string{
		...
		},
		{
			...
			mutate: func(input *stream.PCM16Input) {
				input.ChunkBoundaries = []stream.ChunkBoundary{{ID: "quiet-chunk", SampleIndex: 100}}
...
func TestAnalyzePCM16SyntheticDefects(t *testing.T) {
	for _, test := range syntheticDefectCases() {
		t.Run(test.name, func(t *testing.T) {
			runSyntheticDefectCase(t, test)
		})
	}
}
...
func runSyntheticDefectCase(t *testing.T, test syntheticDefectCase) {
	...
	result, err := stream.AnalyzePCM16(input, stream.DefaultAnalysisConfig())
	if err != nil {
		t.Fatalf("AnalyzePCM16() error = %v", err)
	}
	properties := failureProperties(result.Failures)
	assertExpectedProperties(t, properties, result.Failures, test.wantProperties, test.wantNoProperties)
...
func assertExpectedProperties(t *testing.T, properties map[string]bool, failures []stream.PropertyFailure, want, wantAbsent []string) {
	...
	for _, property := range want {
		if !properties[property] {
			t.Errorf("missing property %q in failures: %v", property, failures)
		}
	}
	for _, property := range wantAbsent {
		if properties[property] {
			t.Errorf("unexpected property %q in failures: %v", property, failures)
		}
	}
}

func assertSyntheticDefectDetails(t *testing.T, name string, result stream.PCM16Analysis) {
	t.Helper()
	switch name {
	case "clean":
		if !result.Passed() {
	...
	case "natural pause":
		assertNaturalPause(t, result)
	case "in-speech dropout":
		assertInSpeechDropout(t, result)
	}
}

func assertNaturalPause(t *testing.T, result stream.PCM16Analysis) {
	t.Helper()
...
func assertInSpeechDropout(t *testing.T, result stream.PCM16Analysis) {
	t.Helper()
...
func TestAnalyzePCM16QuietBoundaryAndLoudImpulseClassification(t *testing.T) {
	...
	quiet.ChunkBoundaries = []stream.ChunkBoundary{{ID: "quiet", SampleIndex: 100}}
	quiet.Samples[100] = 7000
	quietResult, err := stream.AnalyzePCM16(quiet, stream.DefaultAnalysisConfig())
	if err != nil {
	...
	loud.ChunkBoundaries = []stream.ChunkBoundary{{ID: "loud", SampleIndex: 100}}
	for index := 80; index < 120; index++ {
		loud.Samples[index] = 8000
	}
	loud.Samples[100] = 16000
	loudResult, err := stream.AnalyzePCM16(loud, stream.DefaultAnalysisConfig())
	if err != nil {
...
func TestAssertPCM16ReturnsStructuredActionableFailure(t *testing.T) {
	...
	err := stream.AssertPCM16(input, stream.DefaultAnalysisConfig())
	if err == nil {
		t.Fatal("AssertPCM16() error = nil, want clipping failure")
	}
	if !errors.Is(err, stream.ErrPCM16AnalysisFailed) {
		t.Fatalf("AssertPCM16() error = %v, want ErrPCM16AnalysisFailed", err)
	}
	var assertionErr *stream.PCM16AssertionError
...
func TestPCM16AnalysisErrorTypesHandleNilAndEmptyState(t *testing.T) {
	...
	emptyAssertionErr := &stream.PCM16AssertionError{StreamID: "s"}
	...
	var nilInputErr *stream.InvalidPCM16AnalysisInputError
	if got, want := nilInputErr.Error(), "<nil>"; got != want {
		t.Errorf("nil *InvalidPCM16AnalysisInputError.Error() = %q, want %q", got, want)
	}
	fieldlessInputErr := &stream.InvalidPCM16AnalysisInputError{Reason: "broken"}
	if got, want := fieldlessInputErr.Error(), stream.ErrInvalidPCM16AnalysisInput.Error()+": broken"; got != want {
...
func TestPCM16AnalysisConfigRejectsInvalidBounds(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(*stream.PCM16AnalysisConfig)
		field  string
	}{
		{
			name:   "non-positive frame duration",
			mutate: func(config *stream.PCM16AnalysisConfig) { config.FrameDuration = -time.Millisecond },
			field:  "frame_duration",
		},
		{
			name:   "positive silence floor",
			mutate: func(config *stream.PCM16AnalysisConfig) { config.SilenceFloorDBFS = 5 },
			field:  "silence_floor_dbfs",
		},
		{
			name:   "non-positive max natural pause",
			mutate: func(config *stream.PCM16AnalysisConfig) { config.MaxNaturalPause = -time.Millisecond },
			field:  "max_natural_pause",
		},
		{
			name:   "non-positive boundary delta",
			mutate: func(config *stream.PCM16AnalysisConfig) { config.BoundaryDelta = -1 },
			field:  "boundary_delta",
		},
		{
			name:   "non-negative boundary quiet dbfs",
			mutate: func(config *stream.PCM16AnalysisConfig) { config.BoundaryQuietDBFS = 0.5 },
			field:  "boundary_quiet_dbfs",
		},
		{
			name:   "clip threshold above int16 max",
			mutate: func(config *stream.PCM16AnalysisConfig) { config.ClipSampleThreshold = 40000 },
			field:  "clip_sample_threshold",
		},
		{
			name:   "negative edge threshold",
			mutate: func(config *stream.PCM16AnalysisConfig) { config.EdgeSampleThreshold = -1 },
			field:  "edge_sample_threshold",
		},
		{
			name:   "positive final frame max rms",
			mutate: func(config *stream.PCM16AnalysisConfig) { config.FinalFrameMaxRMSDBFS = 5 },
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			config := stream.DefaultAnalysisConfig()
			test.mutate(&config)
			_, err := stream.AnalyzePCM16(cleanAnalysisInput(), config)
			if err == nil || !errors.Is(err, stream.ErrInvalidPCM16AnalysisInput) {
				t.Fatalf("AnalyzePCM16() error = %v, want invalid-input error", err)
			}
			var inputErr *stream.InvalidPCM16AnalysisInputError
...
func TestAnalyzePCM16RejectsInconsistentInputs(t *testing.T) {
	tests := []struct {
		...
		config stream.PCM16AnalysisConfig
		field  string
	}{
		{
			name:  "missing sample rate",
			input: stream.PCM16Input{Samples: []int16{1}},
			field: "sample_rate",
		},
		{
			name:  "empty samples",
			input: stream.PCM16Input{SampleRate: 1000},
			field: "samples",
		},
		{
			name:  "fractional frame",
			input: stream.PCM16Input{SampleRate: 44100, Samples: []int16{1}},
			config: stream.PCM16AnalysisConfig{
		...
		},
		{
			...
			input: stream.PCM16Input{
				...
				ExpectedSpeech: []stream.SpeechAnnotation{{StartSample: 80, EndSample: 101}},
			},
		...
		},
		{
			...
			input: stream.PCM16Input{
				SampleRate:      1000,
				Samples:         make([]int16, 100),
				ChunkBoundaries: []stream.ChunkBoundary{{SampleIndex: 20}, {SampleIndex: 20}},
			},
		...
		},
		{
			...
			input: stream.PCM16Input{
				SampleRate:      1000,
				Samples:         make([]int16, 100),
				ChunkBoundaries: []stream.ChunkBoundary{{SampleIndex: 10}},
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := stream.AnalyzePCM16(test.input, test.config)
			if err == nil || !errors.Is(err, stream.ErrInvalidPCM16AnalysisInput) {
				t.Fatalf("AnalyzePCM16() error = %v, want invalid-input error", err)
			}
			var inputErr *stream.InvalidPCM16AnalysisInputError
...
func cleanAnalysisInput() stream.PCM16Input {
	samples := make([]int16, 2000)
	for index := 200; index < 1600; index++ {
		samples[index] = 6000
	}
	return stream.PCM16Input{
		StreamID:       "clean-stream",
		ParticipantID:  "alice",
		SampleRate:     1000,
		Samples:        samples,
		ExpectedSpeech: []stream.SpeechAnnotation{{Label: "turn-1", StartSample: 200, EndSample: 1600}},
	}
}
