...
func TestReplayLifecycleAcceptsOneCleanSession(t *testing.T) {
	...
	writeTimelineEvents(t, directory, []Event{
		{Version: SchemaVersion, Sequence: 1, Kind: "recording_started", Timestamp: base.Format(time.RFC3339Nano)},
		{Version: SchemaVersion, Sequence: 2, Kind: "runtime", ElapsedNS: int64(time.Millisecond), Timestamp: base.Add(time.Millisecond).Format(time.RFC3339Nano)},
		{Version: SchemaVersion, Sequence: 3, Kind: "recording_closed", ElapsedNS: 2 * int64(time.Millisecond), Clean: true},
	})
	...
	if event, frame, nextErr := replay.Next(); nextErr != nil || frame != nil || event.Kind != "recording_started" {
		t.Fatalf("first event = %#v, frame %v, err %v; want recording_started and nil frame", event, frame, nextErr)
	}
	if event, frame, nextErr := replay.Next(); nextErr != nil || frame != nil || event.Kind != "runtime" {
		t.Fatalf("runtime event = %#v, frame %v, err %v; want runtime and nil frame", event, frame, nextErr)
	}
	if event, frame, nextErr := replay.Next(); nextErr != nil || frame != nil || event.Kind != "recording_closed" {
	...
	tests := []struct {
	...
	}{
		{
			...
			build: func(t *testing.T) string {
				...
				terminal.Kind = "recording_closed"
				terminal.Clean = true
				runtime := Event{Kind: "runtime", ElapsedNS: terminal.ElapsedNS, Timestamp: terminal.Timestamp, RuntimeKind: "after-close"}
		...
		},
		{
			...
			build: func(t *testing.T) string {
				directory := t.TempDir()
				base := time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)
				closeEvent := Event{Kind: "recording_closed", ElapsedNS: int64(time.Millisecond), Timestamp: base.Add(time.Millisecond).Format(time.RFC3339Nano), Clean: true}
				writeTimelineEvents(t, directory, []Event{
					{Kind: "recording_started", Timestamp: base.Format(time.RFC3339Nano)},
					closeEvent,
					closeEvent,
				})
				return directory
		...
		},
		{
			...
			build: func(t *testing.T) string {
				...
				writeTimelineEvents(t, directory, []Event{
					{Kind: "recording_started", Timestamp: base.Format(time.RFC3339Nano)},
					{Kind: "runtime", ElapsedNS: int64(time.Millisecond), Timestamp: base.Add(time.Millisecond).Format(time.RFC3339Nano), RuntimeKind: "inside"},
					{Kind: "recording_started", Timestamp: base.Add(time.Millisecond).Format(time.RFC3339Nano)},
					{Kind: "recording_closed", ElapsedNS: 2 * int64(time.Millisecond), Timestamp: base.Add(2 * time.Millisecond).Format(time.RFC3339Nano), Clean: true},
				})
				return directory
			},
		},
		{
			...
			build: func(t *testing.T) string {
				...
				base := time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)
				writeTimelineEvents(t, directory, []Event{
					{Kind: "runtime", Timestamp: base.Format(time.RFC3339Nano), RuntimeKind: "before-start"},
					{Kind: "recording_closed", ElapsedNS: int64(time.Millisecond), Timestamp: base.Add(time.Millisecond).Format(time.RFC3339Nano), Clean: true},
				})
				return directory
			},
		},
...
func TestReplayLifecycleRejectsValidTraceMutatedAfterFirstClose(t *testing.T) {
	...
	mutated = append(mutated, Event{Kind: "runtime", ElapsedNS: terminal.ElapsedNS, Timestamp: terminal.Timestamp, RuntimeKind: "post-close-runtime"})
	mutated = append(mutated, terminal)
