...
const (
	// OpusSampleRate is the only sample rate supported by this Opus codec.
	OpusSampleRate = 48000
	// OpusChannels is the number of channels in this Opus codec.
	OpusChannels = 1
	// OpusFrameDuration is the duration of one Opus packet.
	OpusFrameDuration = 20 * time.Millisecond
	// OpusFrameSamples is the number of mono samples in one Opus packet.
...
var (
	// ErrOpusInvalidConfiguration identifies an unsupported Opus setup.
...
// OpusCodecConfig describes the deliberately narrow Opus format.
//
...
// OpusEncoder is a serialized, stateful genuine Opus encoder.
...
// NewOpusEncoder constructs an independent 48 kHz mono 20 ms encoder.
func NewOpusEncoder(configs ...OpusCodecConfig) (*OpusEncoder, error) {
...
// OpusDecoder is a serialized, stateful genuine Opus decoder with real PLC.
...
// NewOpusDecoder constructs an independent 48 kHz mono 20 ms decoder.
func NewOpusDecoder(configs ...OpusCodecConfig) (*OpusDecoder, error) {
	config, err := normalizeOpusConfig(configs)
	if err != nil {
		return nil, err
	}
