...
func TestOpusRoundTripProducesNovelPCM(t *testing.T) {
...
func TestOpusPLCUsesVoicedHistoryAndResumesDecode(t *testing.T) {
	encoder, err := NewOpusEncoder()
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpusRejectsInvalidStateAndInputs(t *testing.T) {
	encoder, err := NewOpusEncoder()
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpusConstructorsValidateFormat(t *testing.T) {
	cases := []struct {
		name   string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := NewOpusEncoder(tc.config); !errors.Is(err, ErrOpusInvalidConfiguration) {
				t.Fatalf("encoder error = %v, want ErrOpusInvalidConfiguration", err)
			}
			if _, err := NewOpusDecoder(tc.config); !errors.Is(err, ErrOpusInvalidConfiguration) {
				t.Fatalf("decoder error = %v, want ErrOpusInvalidConfiguration", err)
			}
		})
	}
	if _, err := NewOpusEncoder(OpusCodecConfig{}, OpusCodecConfig{}); !errors.Is(err, ErrOpusInvalidConfiguration) {
		t.Fatalf("multiple configs error = %v, want ErrOpusInvalidConfiguration", err)
	}
}

func TestOpusOutputOwnershipAndIndependentHistory(t *testing.T) {
	encoder, err := NewOpusEncoder()
	if err != nil {
		t.Fatal(err)
	}
	decoderA, err := NewOpusDecoder()
	if err != nil {
		t.Fatal(err)
	}
	decoderB, err := NewOpusDecoder()
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = encoder.Close() }()
...
func TestOpusLifecycleAndOperationErrors(t *testing.T) {
	encoder, err := NewOpusEncoder()
	if err != nil {
		t.Fatal(err)
	}
