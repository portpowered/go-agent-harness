...
func TestVerifyEventsAcceptsExactlyOneNonSkippedCompletion(t *testing.T) {
	...
	for _, testName := range requiredTests {
		output.WriteString(`{"Action":"run","Package":"example/rtc","Test":"`)
		output.WriteString(testName)
		output.WriteString(`"}` + "\n")
		output.WriteString(`{"Action":"pass","Package":"example/rtc","Test":"`)
		output.WriteString(testName)
		output.WriteString(`"}` + "\n")
	}
...
func TestVerifyEventsRejectsMissingRequiredTest(t *testing.T) {
	input := `{"Action":"pass","Package":"example/rtc","Test":"TestPeerS8ConcurrentConnectCloseAndReads"}` + "\n"
	err := verifyEvents(strings.NewReader(input))
	if err == nil || !strings.Contains(err.Error(), "missing required tests") || !strings.Contains(err.Error(), "TestInboundTrackS8ConcurrentIngestReadCancelClose") {
...
func TestVerifyEventsRejectsSkippedRequiredTest(t *testing.T) {
	input := `{"Action":"skip","Package":"example/rtc","Test":"TestOutboundTrackConcurrentWriteCancelClose"}` + "\n"
	err := verifyEvents(strings.NewReader(input))
	if err == nil || !strings.Contains(err.Error(), "skipped required tests") || !strings.Contains(err.Error(), "TestOutboundTrackConcurrentWriteCancelClose") {
...
func TestVerifyEventsRejectsDuplicateCompletion(t *testing.T) {
	...
	for _, action := range []string{"pass", "pass"} {
		input.WriteString(`{"Action":"` + action + `","Package":"example/rtc","Test":"TestPeerS8ConcurrentConnectCloseAndReads"}` + "\n")
	}
