...
const (
	defaultGoBinary  = "go"
	defaultModuleDir = "../../go-llm-gateway"
	defaultTimeout   = 30 * time.Second
	rtcPackage       = "./pkg/transport/rtc"
	runPattern       = "^(TestPeerS8ConcurrentConnectCloseAndReads|TestOutboundTrackSerializesConcurrentWrites|TestOutboundTrackConcurrentWriteCancelClose|TestInboundTrackS8ConcurrentIngestReadCancelClose)$"
)

var requiredTests = []string{
	"TestPeerS8ConcurrentConnectCloseAndReads",
	"TestOutboundTrackSerializesConcurrentWrites",
...
func run(args []string, stdout, stderr io.Writer) error {
	flags := flag.NewFlagSet("rtc-race-gate", flag.ContinueOnError)
	flags.SetOutput(stderr)
	goBinary := flags.String("go", defaultGoBinary, "Go command to execute")
	moduleDir := flags.String("module-dir", defaultModuleDir, "go-llm-gateway module directory")
	timeout := flags.Duration("timeout", defaultTimeout, "finite go test timeout")
	if err := flags.Parse(args); err != nil {
		return err
	}
	if flags.NArg() != 0 {
		return fmt.Errorf("rtc race gate does not accept positional arguments: %s", strings.Join(flags.Args(), " "))
	}
	if strings.TrimSpace(*goBinary) == "" {
		return errors.New("rtc race gate requires a Go command")
	}
	if strings.TrimSpace(*moduleDir) == "" {
		return errors.New("rtc race gate requires a module directory")
	}
	if *timeout <= 0 {
		return fmt.Errorf("rtc race gate timeout must be finite and positive, got %s", *timeout)
	}
...
func execute(cfg config, stdout, stderr io.Writer) error {
	...
	if commandErr != nil {
		if verificationErr != nil {
			return fmt.Errorf("focused RTC race command failed: %v; %w", commandErr, verificationErr)
		}
		return fmt.Errorf("focused RTC race command failed: %w", commandErr)
	}
...
func (f *gateFailure) Error() string {
	...
	return "focused RTC race gate failed; " + strings.Join(parts, "; ")
}
