...
func TestRTCDeviceSourceConvertsSupportedCaptureRateToProviderRate(t *testing.T) {
	...
	feed, err := devicegw.NewDeviceSinkAtRate(registry, "virtual:output", audio.SampleRate)
	if err != nil {
	...
	reference, err := wavio.NewPCM16Resampler(audio.SampleRate, providerRate)
	if err != nil {
		t.Fatal(err)
	}
	wantProvider, err := reference.Process(wantCapture, false)
	if err != nil {
	...
	if held := len(wantCapture)*providerRate/audio.SampleRate - len(outbound.frames[0].Samples); held < 0 || held > 2 {
		t.Fatalf("capture lookahead retained %d output samples, want at most two", held)
	}
}

func TestRTCDeviceSourceKeepsMatchedProviderRateIdentity(t *testing.T) {
	const providerRate = wavio.Rate24kHz
	capability := devicegw.VirtualCapability{SampleRate: providerRate, Channels: audio.Channels, BitDepth: audio.DeviceBitDepthPCM16, Format: audio.DeviceEncodingPCM16}
	registry, err := devicegw.NewVirtualRegistry(devicegw.VirtualBackendConfig{
		Devices: []devicegw.VirtualDeviceConfig{
			{ID: "input", Name: "Input", Direction: devicegw.DirectionInput, Capabilities: []devicegw.VirtualCapability{capability}, LoopbackID: "output"},
			{ID: "output", Name: "Output", Direction: devicegw.DirectionOutput, Capabilities: []devicegw.VirtualCapability{capability}, LoopbackID: "input"},
...
func TestRTCDeviceSourceRejectsUnsupportedCaptureConversionBeforeOpen(t *testing.T) {
	...
	capability := devicegw.VirtualCapability{SampleRate: observedRate, Channels: audio.Channels, BitDepth: audio.DeviceBitDepthPCM16, Format: audio.DeviceEncodingPCM16}
	registry, err := devicegw.NewVirtualRegistry(devicegw.VirtualBackendConfig{
		Devices: []devicegw.VirtualDeviceConfig{{ID: "input", Name: "Input", Direction: devicegw.DirectionInput, Capabilities: []devicegw.VirtualCapability{capability}}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRTCDeviceSourcePreservesRegistryErrorIdentity(t *testing.T) {
	...
	if !errors.As(err, &registryError) || !errors.Is(err, devicegw.ErrNilDeviceRegistry) {
		t.Fatalf("nil registry error = %v, want typed nil-registry error", err)
	}
	if registryError.ID != "virtual:input" || registryError.Direction != devicegw.DirectionInput {
	...
	delete(noDefaultConfig.Defaults, devicegw.DirectionInput)
	...
	if !errors.As(err, &noDefault) || noDefault.Direction != devicegw.DirectionInput || !errors.Is(err, devicegw.ErrNoDefaultDevice) {
