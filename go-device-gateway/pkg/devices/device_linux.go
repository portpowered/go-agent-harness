...
var (
	...
	_ audio.AudioSource = (*linuxOpenedDevice)(nil)
	_ audio.AudioSink   = (*linuxOpenedDevice)(nil)
)
...
func (r *LinuxDeviceRegistry) OpenWithFormat(id DeviceID, format audio.DeviceFormat) (OpenedDevice, error) {
	return r.openAtFormat(id, format, true)
}

func (r *LinuxDeviceRegistry) openAtFormat(id DeviceID, format audio.DeviceFormat, wrapFormatErrors bool) (OpenedDevice, error) {
	...
	if err != nil {
		...
		if wrapFormatErrors {
			return nil, &DeviceFormatError{ID: id, Direction: records[i].Direction, Requested: format, Available: audio.DefaultDeviceFormatAvailability(), Err: mapped}
		}
		return nil, mapped
	}
...
func (r *LinuxDeviceRegistry) openNative(record linuxDeviceRecord, formats ...audio.DeviceFormat) (*linuxOpenedDevice, error) {
	...
	if record.Direction == DirectionOutput {
		var err error
		playback, err = audio.NewPlaybackQueue(format)
		if err != nil {
			return nil, err
		}
	}
...
func (d *linuxOpenedDevice) DeviceFormat() audio.DeviceFormat {
	if d == nil {
...
func (d *linuxOpenedDevice) WriteSamples(ctx context.Context, samples []int16) error {
	...
	defer d.mu.Unlock()
	if d.closed {
		return &audio.ClosedError{Operation: "write", Path: string(d.id)}
	}
...
func (d *linuxOpenedDevice) PlaybackStats() audio.PlaybackQueueStats {
	if d == nil {
	...
	defer d.mu.Unlock()
	if d.playback == nil {
		return audio.EmptyPlaybackQueueStats(d.format)
	}
	return d.playback.Snapshot()
}

func (d *linuxOpenedDevice) SetPlaybackRenderObserver(observer audio.PlaybackRenderObserver) {
...
func (d *linuxOpenedDevice) CaptureStats() audio.CaptureQueueStats {
...
func (d *linuxOpenedDevice) WaitForPlaybackCapacity(ctx context.Context, samples int) error {
	...
	for {
		...
		if d.closed {
			d.mu.Unlock()
			return &audio.ClosedError{Operation: "wait for playback capacity", Path: string(d.id)}
		}
...
func (d *linuxOpenedDevice) WaitForPlayback(ctx context.Context) error {
	...
	for {
		...
		if d.closed {
			d.mu.Unlock()
			return &audio.ClosedError{Operation: "wait for playback", Path: string(d.id)}
		}
		queued := 0
		if d.playback != nil {
