...
func TestVoiceProcessingRenderMarksQueueUnderflowAsSilence(t *testing.T) {
	queue, err := audio.NewPlaybackQueue(audio.DefaultDeviceFormat())
	if err != nil {
		t.Fatal(err)
	}
	engine := &voiceProcessingIO{outputFormat: audio.DefaultDeviceFormat(), playback: queue, playbackWake: make(chan struct{})}
	...
	var flags uint32
	if status := engine.renderBuffers(&flags, audio.FrameSize, &list); status != 0 {
...
func TestVoiceProcessingPlaybackCapacityWaitResumesAtLowWatermark(t *testing.T) {
	...
	low, high, err := audio.PlaybackQueueWatermarks(format)
	if err != nil {
		t.Fatal(err)
	}
	for queued := 0; queued < high; queued += audio.FrameSize {
		if err := endpoint.WriteFrame(context.Background(), make([]int16, audio.FrameSize)); err != nil {
			t.Fatalf("prime AUVoiceIO playback queue: %v", err)
		}
	}
	waitDone := make(chan error, 1)
	go func() { waitDone <- endpoint.WaitForPlaybackCapacity(context.Background(), audio.FrameSize) }()
	select {
	case err := <-waitDone:
...
func TestVoiceProcessingPlaybackBurstPreservesFIFO(t *testing.T) {
	...
	testPacedPlaybackBackend(t, endpoint, func(raw []byte) {
		list := audioBufferList1{NumberBuffers: 1, Buffers: [1]audioBuffer{{NumberChannels: 1, DataByteSize: uint32(len(raw)), Data: unsafe.Pointer(&raw[0])}}}
		if status := engine.renderBuffers(nil, audio.FrameSize, &list); status != 0 {
...
func TestVoiceProcessingPlaybackCapacityWaitLifecycle(t *testing.T) {
	newBlockedWait := func(t *testing.T) (*voiceProcessingIO, *voiceProcessingEndpoint, <-chan error) {
		...
		engine.refs.Store(2)
		endpoint := &voiceProcessingEndpoint{engine: engine, id: "coreaudio:voice-processing:output", direction: DirectionOutput, format: format}
		_, high, err := audio.PlaybackQueueWatermarks(format)
		if err != nil {
			t.Fatal(err)
		}
		for queued := 0; queued < high; queued += audio.FrameSize {
...
func TestCoreAudioPlaybackQueueUsesResolvedRateAndCountsOverflow(t *testing.T) {
	...
	handle.onData(output, nil, audio.FrameSize)
...
func TestCoreAudioPlaybackCapacityWaitResumesAtLowWatermark(t *testing.T) {
	handle := &coreAudioHandle{direction: DirectionOutput, format: audio.DefaultDeviceFormat(), playbackWake: make(chan struct{})}
	low, high, err := audio.PlaybackQueueWatermarks(handle.format)
	if err != nil {
		t.Fatalf("resolve playback watermarks: %v", err)
	}
	for queued := 0; queued < high; queued += audio.FrameSize {
		if err := handle.WriteFrame(context.Background(), make([]int16, audio.FrameSize)); err != nil {
			t.Fatalf("prime playback queue: %v", err)
		}
	}

	waitDone := make(chan error, 1)
	go func() { waitDone <- handle.WaitForPlaybackCapacity(context.Background(), audio.FrameSize) }()
	select {
	case err := <-waitDone:
	...
	for handle.PlaybackStats().QueuedSamples > low {
		handle.onData(callback, nil, audio.FrameSize)
	}
	select {
	case err := <-waitDone:
		if err != nil {
...
func TestCoreAudioPlaybackBurstPreservesFIFO(t *testing.T) {
	handle := &coreAudioHandle{direction: DirectionOutput, format: audio.PCM16DeviceFormat(24000), playbackWake: make(chan struct{})}
	testPacedPlaybackBackend(t, handle, func(raw []byte) {
		handle.onData(raw, nil, audio.FrameSize)
	})
}
