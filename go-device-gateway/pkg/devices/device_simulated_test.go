...
func TestSimulatedDuplexDeviceLossWakesBlockedRead(t *testing.T) {
	...
	go func() { done <- input.ReadFrame(context.Background(), make([]int16, audio.FrameSize)) }()
	if err := r.Advance(1); err != nil {
		t.Fatal(err)
	}
	select {
...
func TestSimulatedDuplexRegistryAndStreamContracts(t *testing.T) {
	...
	if err := in.WriteFrame(context.Background(), make([]int16, audio.FrameSize)); err == nil {
		t.Fatal("input accepted write")
	}
	if err := out.ReadFrame(context.Background(), make([]int16, audio.FrameSize)); err == nil {
	...
	if err != nil || len(raw) != audio.FrameSize*2 {
		t.Fatalf("Read = %d/%v", len(raw), err)
	}
	decoded := make([]int16, audio.FrameSize)
	if err := codec.DecodePCM16Into(decoded, raw); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(decoded, frame) {
		t.Fatal("raw round trip changed PCM")
	}
	rawFrame := make([]byte, audio.FrameSize*2)
	if err := codec.EncodePCM16Into(rawFrame, frame); err != nil {
		t.Fatal(err)
	}
	if err := out.Write(context.Background(), rawFrame); err != nil {
		t.Fatal(err)
	}
	if out.PlaybackStats().QueuedSamples != audio.FrameSize {
		t.Fatalf("queued = %+v", out.PlaybackStats())
	}
	if err := out.WaitForPlaybackCapacity(context.Background(), audio.FrameSize); err != nil {
		t.Fatal(err)
	}
	if err := out.WaitForPlaybackCapacity(context.Background(), 100000); !errors.Is(err, audio.ErrInvalidPlaybackQueue) {
		t.Fatalf("oversized capacity = %v", err)
	}
	if got := out.DiscardPlayback(); got != audio.FrameSize {
