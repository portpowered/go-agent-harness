...
type SimulatedDuplexRegistry struct {
	...
	observedPlayback                audio.PlaybackQueueStats
	observedCapture                 audio.CaptureQueueStats
}
...
func NewSimulatedDuplexRegistryWithObservability(s DuplexScenario, sampler observability.MetricSampler, logger observability.Logger) (*SimulatedDuplexRegistry, error) {
	...
	format := audio.PCM16DeviceFormat(s.Render.NominalRate)
	latency := audio.DefaultPlaybackLatencyTarget
	if s.PlaybackQueue.LatencyNanos > 0 {
		latency = durationFromNanos(s.PlaybackQueue.LatencyNanos)
	}
	playback, err := audio.NewPlaybackQueueWithLatency(format, latency)
	if err != nil {
		return nil, err
	}
	captureCapacity, err := audio.PlaybackQueueCapacity(format, audio.DefaultPlaybackLatencyTarget)
	if err != nil {
		return nil, err
	}
	if s.CaptureQueue.LatencyNanos > 0 {
		captureCapacity, err = audio.PlaybackQueueCapacity(format, durationFromNanos(s.CaptureQueue.LatencyNanos))
		if err != nil {
			return nil, err
		}
	}
	...
	return &SimulatedDuplexRegistry{
		...
		captureStats:  audio.CaptureQueueStats{DropPolicy: s.CaptureQueue.DropPolicy},
...
func (r *SimulatedDuplexRegistry) OpenWithFormat(id DeviceID, f audio.DeviceFormat) (OpenedDevice, error) {
	if !f.Equal(r.format) {
		return nil, &DeviceFormatError{ID: id, Requested: f, Available: []audio.DeviceFormat{r.format}}
	}
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (s *SimulatedDuplexStream) DeviceFormat() audio.DeviceFormat { return s.registry.format }
...
func (s *SimulatedDuplexStream) Write(ctx context.Context, raw []byte) error {
	...
	if err := codec.DecodePCM16Into(samples, raw); err != nil {
		return err
	}
	return s.WriteSamples(ctx, samples)
}
func (s *SimulatedDuplexStream) PlaybackStats() audio.PlaybackQueueStats {
	return s.registry.playback.Snapshot()
}

func (s *SimulatedDuplexStream) SetPlaybackRenderObserver(observer audio.PlaybackRenderObserver) {
	if s == nil || s.registry == nil || s.direction != DirectionOutput {
		return
	}
	s.registry.playback.SetRenderObserver(observer)
}
func (s *SimulatedDuplexStream) CaptureStats() audio.CaptureQueueStats {
...
func (s *SimulatedDuplexStream) WaitForPlaybackCapacity(ctx context.Context, samples int) error {
	...
	_, high, err := audio.PlaybackQueueWatermarks(s.registry.format)
	if err != nil {
		return err
	}
	if samples > high {
...
func (r *SimulatedDuplexRegistry) observeQueueDeltas(playback, previousPlayback audio.PlaybackQueueStats, capture, previousCapture audio.CaptureQueueStats) {
...
func (r *SimulatedDuplexRegistry) enqueueCaptureLocked(block []int16) {
	...
	r.captureStats.CompletedFrames += uint64(assembled / audio.FrameSize)
	r.captureRemainder = assembled % audio.FrameSize
...
func (r *SimulatedDuplexRegistry) CaptureStats() audio.CaptureQueueStats {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.captureStats
}
func (r *SimulatedDuplexRegistry) PlaybackStats() audio.PlaybackQueueStats {
	return r.playback.Snapshot()
}
