...
func TestDeviceSinkVirtualFramesAndLoss(t *testing.T) {
	...
	wants := make([][]int16, 3)
	for i := range wants {
		wants[i] = make([]int16, audio.FrameSize)
		wants[i][0], wants[i][1], wants[i][audio.FrameSize-1] = int16(i+1), -32768, 32767-int16(i)
	...
	err = sinkContract.WriteFrame(context.Background(), make([]int16, audio.FrameSize))
...
func TestDeviceSinkFrameHandleConformance(t *testing.T) {
	...
	for i, frame := range handle.writes {
		if frame[0] != int16(i+1) || frame[audio.FrameSize-1] != int16(32766-i) {
			t.Fatalf("native frame %d = (%d,%d), want (%d,%d)", i, frame[0], frame[audio.FrameSize-1], i+1, 32766-i)
		}
	}
}
...
func TestDeviceSinkWriteSamplesQueuesExactPartialChunk(t *testing.T) {
	...
	if err := frameSink.WriteSamples(context.Background(), make([]int16, audio.FrameSize)); err != nil {
	...
	if err := byteSink.WriteSamples(context.Background(), []int16{1}); !errors.Is(err, audio.ErrClosed) {
...
func TestDeviceSinkFormatCapabilitiesAndExplicitFormatFailures(t *testing.T) {
	...
	if stats.Format != audio.DefaultDeviceFormat() || stats.CapacitySamples != 4000 {
	...
	if got := sink.SampleRate(); got != audio.SampleRate {
		t.Fatalf("legacy sink SampleRate = %d, want %d", got, audio.SampleRate)
	}
	...
	if !errors.As(err, &formatErr) || !errors.Is(err, audio.ErrUnsupportedDeviceFormat) || !strings.Contains(err.Error(), "24000 Hz") {
	...
	if nilSink.DeviceFormat() != (audio.DeviceFormat{}) || nilSink.SampleRate() != 0 || nilSink.PlaybackStats() != (audio.PlaybackQueueStats{}) || nilSink.DiscardPlayback() != 0 {
...
func TestDeviceSinkExplicitOpenerValidatesOpenedFormat(t *testing.T) {
	...
	actual := audio.DefaultDeviceFormat()
...
func TestDeviceSinkContractsAndConcurrentClose(t *testing.T) {
	...
	cancel()
	if err := sink.WriteFrame(ctx, make([]int16, audio.FrameSize)); !errors.Is(err, context.Canceled) {
		t.Fatalf("cancelled WriteFrame = %v", err)
	}
	if err := sink.WriteFrame(context.Background(), make([]int16, audio.FrameSize-1)); !errors.Is(err, audio.ErrInvalidFrameSize) {
	...
	go func() { writes <- sink.WriteFrame(context.Background(), make([]int16, audio.FrameSize)) }()
	waitDeviceSignal(t, blocking.started)
	closeDone := make(chan error, 1)
	go func() { closeDone <- sink.Close() }()
	if err := waitDeviceError(t, writes); !errors.Is(err, audio.ErrClosed) {
...
func (h *adapterFormatHandle) DeviceFormat() audio.DeviceFormat { return h.format }
...
func (r *adapterFormatRegistryStub) OpenWithFormat(_ DeviceID, format audio.DeviceFormat) (OpenedDevice, error) {
	r.requested = format
	return r.handle, nil
}
