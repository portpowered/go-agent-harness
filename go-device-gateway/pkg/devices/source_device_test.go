...
func TestDeviceSourceConformanceAndValidation(t *testing.T) {
	...
	samples[0], samples[audio.FrameSize] = -32768, 32767
	handle := &adapterFrameHandle{direction: DirectionInput, frames: [][]int16{samples[:audio.FrameSize], samples[audio.FrameSize:]}}
	source, err := NewDeviceSource(&adapterTestRegistryStub{handle: handle}, "input")
	if err != nil {
		t.Fatal(err)
	}
	var contract audio.AudioSource = source
	assertSourceFrames(t, contract, samples)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if err := source.ReadFrame(ctx, make([]int16, audio.FrameSize)); !errors.Is(err, context.Canceled) {
		t.Fatalf("cancelled ReadFrame = %v", err)
	}
	if err := source.ReadFrame(context.Background(), make([]int16, audio.FrameSize-1)); !errors.Is(err, audio.ErrInvalidFrameSize) {
...
func TestDeviceSourceVirtualLossAndConcurrentClose(t *testing.T) {
	...
	go func() {
		result <- source.ReadFrame(&adapterReadyContext{Context: context.Background(), ready: ready}, make([]int16, audio.FrameSize))
	}()
	...
	if closeErr != nil || !errors.Is(readErr, audio.ErrClosed) {
