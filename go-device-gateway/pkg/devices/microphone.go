...
func NewMicrophoneSource() (*MicrophoneSource, error) {
	...
	cfg.Capture.Channels = uint32(audio.Channels)
	cfg.SampleRate = uint32(audio.SampleRate)
...
func (m *MicrophoneSource) onCapture(raw []byte, frames int) {
	...
	if err := codec.DecodePCM16Into(samples, raw[:frames*2]); err != nil {
		return
	}
	...
	for len(m.pending) >= audio.FrameSize {
		frame := make([]int16, audio.FrameSize)
		copy(frame, m.pending[:audio.FrameSize])
		m.pending = m.pending[audio.FrameSize:]
		...
		default:
			...
			m.stats.DroppedSamples += audio.FrameSize
			m.stats.SequenceGaps++
		}
		m.stats.QueuedSamples = len(m.frameCh) * audio.FrameSize
...
func (m *MicrophoneSource) CaptureStats() audio.CaptureQueueStats {
	if m == nil {
	...
	defer m.mu.Unlock()
	stats := m.stats
	stats.QueuedSamples = len(m.frameCh) * audio.FrameSize
	return stats
}
