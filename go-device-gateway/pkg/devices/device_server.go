...
type remoteOpenRequest struct {
	...
	Format   audio.DeviceFormat `json:"format"`
}
...
type remoteErrorResponse struct {
	...
	Requested audio.DeviceFormat   `json:"requested,omitempty"`
...
func (s *DeviceServer) handleOpen(w http.ResponseWriter, r *http.Request) {
	...
	} else if request.Format.Equal(audio.DefaultDeviceFormat()) {
		handle, err = s.registry.Open(request.DeviceID)
	} else {
		err = &DeviceFormatError{ID: request.DeviceID, Direction: device.Direction, Requested: request.Format, Available: audio.DefaultDeviceFormatAvailability()}
	}
	if err != nil {
...
func (s *DeviceServer) handleRead(w http.ResponseWriter, r *http.Request, handle OpenedDevice) {
	...
	if err != nil || count != audio.FrameSize {
		writeDeviceServerError(w, http.StatusBadRequest, &audio.FrameSizeError{Operation: "remote device read", Got: count, Want: audio.FrameSize})
		return
	}
...
func (s *DeviceServer) handleWrite(w http.ResponseWriter, r *http.Request, handle OpenedDevice) {
	...
	if err := codec.DecodePCM16Into(samples, data); err != nil {
		writeDeviceServerError(w, http.StatusBadRequest, fmt.Errorf("remote device write: %w", err))
		return
	}
...
func readDeviceServerPCM16(reader io.Reader) ([]byte, error) {
	...
	if err := codec.ValidatePCM16(data, deviceServerPCMLimit); err != nil {
		return nil, fmt.Errorf("PCM16 payload validation: %w", err)
	}
	return data, nil
}
...
func (r *RemoteDeviceRegistry) OpenWithFormat(id DeviceID, format audio.DeviceFormat) (OpenedDevice, error) {
...
func (d *remoteOpenedDevice) ReadFrame(ctx context.Context, frame []int16) error {
	...
	if len(data) != len(frame)*2 {
		return &audio.FrameSizeError{Operation: "remote device read", Got: len(data) / 2, Want: len(frame)}
	}
	if err := codec.DecodePCM16Into(frame, data); err != nil {
		return err
	}
	return nil
}
...
func (d *remoteOpenedDevice) PlaybackStats() audio.PlaybackQueueStats {
	var stats audio.PlaybackQueueStats
...
func (d *remoteOpenedDevice) CaptureStats() audio.CaptureQueueStats {
	var stats audio.CaptureQueueStats
