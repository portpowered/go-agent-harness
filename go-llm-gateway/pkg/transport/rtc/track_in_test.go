...
func TestInboundTrackCompatibilityAdapter(t *testing.T) {
	t.Run("defaults and resampled ownership", testInboundResampledOwnership)
	t.Run("identity rate and close identity", testInboundIdentityRateAndClose)
}

func testInboundResampledOwnership(t *testing.T) {
...
func TestInboundTrackCompatibilityReadErrors(t *testing.T) {
	t.Run("canceled context", testInboundCanceledContext)
	t.Run("source errors and cancellation during read", testInboundSourceErrors)
	t.Run("packet validation", testInboundPacketValidation)
	t.Run("decoder and resampler errors", testInboundDecoderAndResamplerErrors)
}

func testInboundCanceledContext(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	track := newCompatibilityInboundTrack(t, &compatibilityInboundSource{}, &compatibilityInboundDecoder{samples: make([]int16, 960)}, InboundTrackConfig{})
	if _, err := track.ReadFrame(ctx); !errors.Is(err, context.Canceled) {
		t.Fatalf("ReadFrame(canceled) = %v, want context.Canceled", err)
	}
}

func testInboundSourceErrors(t *testing.T) {
	readErr := errors.New("read failed")
	track := newCompatibilityInboundTrack(t, &compatibilityInboundSource{reads: []compatibilityInboundRead{{err: readErr}}}, &compatibilityInboundDecoder{samples: make([]int16, 960)}, InboundTrackConfig{})
	...
	ctx, cancel := context.WithCancel(context.Background())
	track = newCompatibilityInboundTrack(t, &compatibilityInboundSource{
		onRead: func() { cancel() },
...
func testInboundPacketValidation(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			track := newCompatibilityInboundTrack(t, &compatibilityInboundSource{reads: []compatibilityInboundRead{{packet: test.read}}}, &compatibilityInboundDecoder{samples: make([]int16, 960)}, InboundTrackConfig{})
...
func testInboundDecoderAndResamplerErrors(t *testing.T) {
	...
	track = newCompatibilityInboundTrack(t, &compatibilityInboundSource{reads: []compatibilityInboundRead{{packet: validPacket}}}, &compatibilityInboundDecoder{samples: make([]int16, 1)}, InboundTrackConfig{})
	...
	track = newCompatibilityInboundTrack(t, &compatibilityInboundSource{reads: []compatibilityInboundRead{{packet: validPacket}}}, &compatibilityInboundDecoder{samples: make([]int16, 960)}, InboundTrackConfig{
		SampleRate: wavio.Rate24kHz,
