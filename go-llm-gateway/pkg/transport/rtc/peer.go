...
// ContextDialer is required so Close can cancel and join an in-flight dial.
...
func (p *Peer) start(reconnect bool, cause error) (*operation, bool, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	if reconnect {
		...
		old, p.conn, p.attempts = p.conn, nil, 0
