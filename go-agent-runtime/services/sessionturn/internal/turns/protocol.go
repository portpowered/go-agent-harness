...
func sendTurnInput(ctx context.Context, connection messages.Session, input sessionturn.TurnInput) error {
	...
	if err := sendMessage(ctx, connection, messages.StreamMessage{Type: inputMessageType(input), Value: value}, sessionturn.ErrTurnInputRejected, "send turn input"); err != nil {
		return err
	}
	if len(input.Audio) == 0 {
		return nil
	...
	return sendMessage(ctx, connection, commit, sessionturn.ErrTurnInputCommitRejected, "commit turn input")
}

func inputMessageType(input sessionturn.TurnInput) messages.StreamMessageType {
	if len(input.Audio) != 0 {
...
func readTurnResponse(ctx context.Context, connection messages.Session) (messages.Message, error) {
	...
	if buffer == nil {
		return messages.Message{}, fmt.Errorf("read turn: %w", sessionturn.ErrMissingTurnSession)
	}
...
func nextResponseMessage(ctx context.Context, connection messages.Session, buffer *messages.TypedBuffer[messages.StreamMessage]) (messages.StreamMessage, error) {
	...
	case <-connection.Done():
		...
		return messages.StreamMessage{}, transitionError("read", sessionturn.ErrSessionClosed)
	}
}
