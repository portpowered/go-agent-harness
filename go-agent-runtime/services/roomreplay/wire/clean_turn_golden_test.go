...
func TestCleanTurnTakingRoomReplayFixturePassesAudioProperties(t *testing.T) {
	...
	for _, participant := range bundle.Participants {
		assertCleanParticipantEvidence(t, bundle, participant)
	}
	...
	for _, stream := range analysis.Streams {
		assertCleanStreamFacts(t, stream)
	}
}
...
func assertCleanStreamDeltas(t *testing.T, participantID string, stream roomreplay.RoomReplayAudioStream) {
	...
	if stream.Role == "wav" && (!bytes.Equal(reconstructed, stream.PCM) || stream.SampleCount != len(reconstructed)/2) {
		t.Fatalf("participant %q output deltas do not reproduce %d PCM bytes/%d samples", participantID, len(stream.PCM), stream.SampleCount)
	}
}

func assertCleanStreamFacts(t *testing.T, stream streamanalysis.PCM16Analysis) {
	t.Helper()
