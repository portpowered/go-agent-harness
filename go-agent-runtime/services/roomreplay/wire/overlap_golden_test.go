...
func TestDeliberateOverlapRoomReplayFixturePassesPeerDeliveryAndSelfHearing(t *testing.T) {
	...
	assertOverlapAnnotation(t, bundle)
	assertOverlapProvenance(t)
	for _, participant := range bundle.Participants {
		assertOverlapParticipantEvidence(t, bundle, participant)
	}
	...
	assertOverlapRelationships(t, analysis)
...
func assertOverlapProvenance(t *testing.T) {
	t.Helper()
	...
	for _, want := range []string{
		...
		"--shape deliberate-overlap --output go-agent-runtime/services/roomreplay/wire/testdata/room-audio/deliberate-overlap",
	} {
...
func assertOverlapTimelineBoundaries(t *testing.T, timeline []roomreplay.RoomReplayTimelineEvent, participantID string) {
	...
	for _, event := range timeline {
		...
		foundStart = foundStart || event.OffsetMS == 1000
		foundEnd = foundEnd || event.OffsetMS == 6500
	}
...
func assertOverlapRelationships(t *testing.T, analysis roomanalysis.PCM16RoomAnalysis) {
	t.Helper()
...
func assertOverlapStreams(t *testing.T, analysis roomanalysis.PCM16RoomAnalysis) {
	t.Helper()
...
func TestDeliberateOverlapRoomReplayFixtureDefectsFailWithDirectionAndNumbers(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			assertOverlapDefectFails(t, test)
		})
	}
}
