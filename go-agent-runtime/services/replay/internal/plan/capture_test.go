...
func TestInspectCaptureRetainsLifecycleMetadataForCallerDrivenActions(t *testing.T) {
	for _, test := range []struct {
		name   string
		action gatewaytesting.CapturedSessionEvent
	}{
		{name: "caller cancellation", action: clientRecord("response.cancel", `{"type":"response.cancel"}`)},
		{name: "image plus audio prompt", action: clientRecord("conversation.item.create", `{"type":"conversation.item.create","item":{"type":"message","role":"user","content":[{"type":"input_text","text":"describe this"},{"type":"input_image","image_url":"data:image/png;base64,AA=="}]}}`)},
		{name: "audio without commit", action: clientRecord("input_audio_buffer.append", `{"type":"input_audio_buffer.append","audio":"AQACAA=="}`)},
	} {
