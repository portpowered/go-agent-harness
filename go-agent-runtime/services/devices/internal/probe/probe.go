...
func runDeviceProbeScenario(ctx context.Context, scenario probe.Scenario, availability devicegw.DeviceProbeAvailability, registry devicegw.DeviceRegistry, opts deviceProbeRuntimeOptions, rtcTransport rtctransport.Service, sessionFactory runtimeDevices.ProbeSessionFactory) (observation probe.ObservationSnapshot, runErr error) {
	...
	resources, err := openLiveDeviceProbeResources(registry, inputDevice, outputDevice, rtcTransport)
	if err != nil {
		return observation, err
	}
