...
func loadRoomReplayWAVStream(plan RoomReplayPlan, artifact RoomReplayArtifact, streamID, participantID, role string) (AudioStream, error) {
	...
	if len(wav.PCM) == 0 {
		return AudioStream{}, roomReplayAudioIncomplete("artifact."+role, artifact.Path, "non-empty WAV PCM payload", "empty", ErrRoomReplayBundleIncomplete)
	}
...
func loadRoomReplayPCMStream(plan RoomReplayPlan, artifact RoomReplayArtifact, streamID, participantID, role string) (AudioStream, error) {
	...
	if len(pcm)%(2*channels) != 0 {
		return AudioStream{}, roomReplayAudioMismatch("artifact."+role, artifact.Path, "PCM16 frame-aligned payload", fmt.Sprintf("%d bytes", len(pcm)), nil)
	}
...
func applyRoomReplayStreamMetadata(stream *AudioStream, metadata roomReplayAudioStreamMetadata) {
	if stream == nil {
		return
	}
