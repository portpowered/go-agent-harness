...
func parseRoomReplayAudioAnnotations(manifest roomReplayJSONObject, plan RoomReplayPlan, participants []AudioParticipant, streamParticipants map[string]string) ([]AudioAnnotation, []roomanalysis.PCM16OverlapInterval, []roomanalysis.PCM16BargeInAnnotation, []roomanalysis.PCM16LoudnessInterval, error) {
	...
	participantByID := make(map[string]AudioParticipant, len(participants))
...
func validateRoomReplayAnnotationParticipants(id string, IDs []string, participants map[string]AudioParticipant) error {
...
func roomReplayAnnotationStreams(object roomReplayJSONObject, endpoint, participantID string, participants map[string]AudioParticipant, streamParticipants map[string]string) (string, string, error) {
