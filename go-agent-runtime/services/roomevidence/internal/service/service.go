...
func (s *Service) Load(plan RoomReplayPlan) (roomevidence.Bundle, error) {
...
func loadRoomReplayParticipants(plan RoomReplayPlan, objects map[string]roomReplayJSONObject) ([]AudioParticipant, map[string]string, error) {
	participants := make([]AudioParticipant, 0, len(plan.Participants))
...
func registerRoomReplayStreams(owners map[string]string, participant AudioParticipant, owner string) error {
	for _, stream := range []AudioStream{participant.WAV, participant.Sent, participant.Received} {
...
func loadRoomReplayMix(plan RoomReplayPlan, owners map[string]string) (AudioStream, error) {
	artifact, ok := findRoomReplayArtifact(plan.Artifacts, "room:mix")
	if !ok {
		return AudioStream{}, roomReplayAudioIncomplete("artifacts.room_mix", "", "validated room mix artifact", "missing", ErrRoomReplayBundleIncomplete)
	}
	...
	if previous, exists := owners[mix.StreamID]; exists {
		return AudioStream{}, roomReplayAudioMismatch("streams."+mix.StreamID, "run-manifest.json", "unique stream identity", previous+" and room", nil)
	}
