...
func mismatch(field string, err error) error {
	return &roomevidence.RoomReplayBundleError{Kind: roomevidence.RoomReplayBundleMismatch, Field: field, Err: fmt.Errorf("%w: %w", rooms.ErrInvalidReplayBundle, err)}
}

func incomplete(field string, err error) error {
	return &roomevidence.RoomReplayBundleError{Kind: roomevidence.RoomReplayBundleIncomplete, Field: field, Err: fmt.Errorf("%w: %w", rooms.ErrReplayBundleIncomplete, err)}
}
