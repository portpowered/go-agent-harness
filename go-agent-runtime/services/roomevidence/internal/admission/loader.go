// Package admission admits finalized room evidence and produces an immutable
...
func (Loader) Load(bundle string) (rooms.RoomReplayPlan, error) {
	...
	metadata, err := parseArtifactMetadata(object)
	if err != nil {
		return rooms.RoomReplayPlan{}, err
	}
...
func parseHeader(object object, root, manifestPath string) (rooms.RoomReplayPlan, error) {
	...
	if timestampErr != nil {
		if timing, ok := object["timing"]; ok {
			if timingObject, decodeErr := decodeObject(timing); decodeErr == nil {
				clockBase, timestampErr = timestamp(timingObject, "clock_base")
			}
		}
	}
	if timestampErr != nil {
...
func parseParticipant(value object, id string) (rooms.RoomReplayParticipant, []artifactRef, error) {
	...
	kind := normalizeParticipantKind(rooms.ParticipantKind(stringValue(value, "kind")))
	...
	// Older finalized bundles keep the provider capture beside the artifact
	// projection rather than inside participants.artifacts. Normalize both
	// admitted shapes into the same service-owned participant artifact list.
	if capture, exists := value["capture"]; exists {
		if _, alreadyProjected := artifactValues["capture"]; !alreadyProjected {
			artifactValues["capture"] = capture
		}
	}
...
func parseArtifactMetadata(value object) (map[string]artifactMetadata, error) {
	raw, ok := value["artifact_integrity"]
	if !ok {
		return nil, nil
	}
	entries, err := decodeObject(raw)
	if err != nil {
		return nil, mismatch("artifact_integrity", err)
	}
	metadata := make(map[string]artifactMetadata, len(entries))
	for path, rawEntry := range entries {
		entry, err := decodeObject(rawEntry)
		if err != nil {
			return nil, mismatch("artifact_integrity."+path, err)
		}
		size, ok := integer64(entry, "size")
		if !ok || size < 0 {
			return nil, mismatch("artifact_integrity."+path, fmt.Errorf("size is missing or invalid"))
		}
		digest := strings.ToLower(strings.TrimSpace(stringValue(entry, "sha256")))
		if digest == "" {
			return nil, incomplete("artifact_integrity."+path, fmt.Errorf("sha256 is missing"))
		}
		metadata[filepath.ToSlash(filepath.Clean(path))] = artifactMetadata{Size: size, SHA256: digest}
	}
	return metadata, nil
}
