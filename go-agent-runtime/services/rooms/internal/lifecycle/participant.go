...
func (r Runner) openParticipant(ctx context.Context, state *runState, participant rooms.Participant, request rooms.RoomRunOptions, recorder rooms.EvidenceRecorder) (*activeParticipant, error) {
...
func (r Runner) openAgent(ctx context.Context, state *runState, participant rooms.Participant, request rooms.RoomRunOptions, recorder rooms.EvidenceRecorder, local rooms.MediaPorts) (*activeParticipant, error) {
	...
	if recorder != nil && strings.TrimSpace(liveRequest.Replay.OutputCapturePath) == "" {
		if artifact := recorder.Artifacts(participant.ID).Capture; artifact != "" {
			liveRequest.Replay.OutputCapturePath = filepath.Join(recorder.Destination(), artifact)
		}
	}
