...
func (r Runner) openAgent(ctx context.Context, state *runState, participant rooms.Participant, request rooms.RoomRunOptions, recorder roomevidence.Recorder, local rooms.MediaPorts) (*activeParticipant, error) {
	...
	if recorder != nil && strings.TrimSpace(liveRequest.Replay.OutputCapturePath) == "" {
		if capture := recorder.Artifacts(participant.ID).Capture; capture != "" {
			liveRequest.Replay.OutputCapturePath = filepath.Join(recorder.Destination(), filepath.FromSlash(capture))
		}
	}
