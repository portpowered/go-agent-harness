...
func audioDelta(content []byte) messages.StreamMessage {
	return messages.StreamMessage{
		...
		Value: messages.NewAudioDeltaValue(content),
	}
}
...
func textDelta(content string) messages.StreamMessage {
	return messages.StreamMessage{
		...
		Value: messages.NewTextDeltaValue(content),
	}
}

func TestPublicServicePreservesFrozenJSONLAndTiming(t *testing.T) {
	clock := &testClock{now: time.Date(2026, 8, 30, 12, 0, 0, 0, time.UTC)}
	reducer := conversationlogwire.NewService(clock)
	...
	reducer.Observe(messages.StreamMessage{
		Type:  messages.StreamTypeInputItemAdded,
		Role:  messages.RoleUser,
		Value: messages.NewInputItemAddedValue("item-1"),
	}, false, -1, -1)
	reducer.Observe(messages.StreamMessage{
		Type:  messages.StreamTypeTranscriptDelta,
		Role:  messages.RoleUser,
		Value: messages.NewTranscriptDeltaValueForItem("spoken ", "item-1"),
	}, false, -1, -1)
	...
	reducer.Observe(messages.StreamMessage{
		...
		Value: messages.NewToolCallStartValue("call-1", "inspect_tab"),
	}, false, -1, -1)
	...
	reducer.Observe(messages.StreamMessage{
		...
		Role:  messages.RoleAssistant,
		Value: messages.NewTranscriptEndValue("full response"),
	}, false, -1, -1)
	...
	// The completed item transcript arrives after the assistant turn has
	// closed; the public reducer must still join it by item identity.
	reducer.Observe(messages.StreamMessage{
		Type:  messages.StreamTypeTranscriptEnd,
		Role:  messages.RoleUser,
		Value: messages.NewTranscriptEndValueForItem("spoken full", "item-1"),
	}, false, -1, -1)
	...
	want := "{\"turn_index\":1,\"input\":{\"text\":\"spoken full\",\"audio_bytes\":3,\"committed\":true,\"audio_segments\":[\"audio/in-000.pcm\",\"audio/in-001.pcm\"]}," +
		"\"response\":{\"text\":\"full response\",\"complete\":true,\"audio_bytes\":3,\"audio_segments\":[\"audio/out-000.pcm\",\"audio/out-001.pcm\"]},\"tool_events\":[" +
		"{\"sequence\":1,\"type\":\"tool_call\",\"tool_call_id\":\"call-1\",\"tool_name\":\"inspect_tab\",\"arguments\":\"{\\\"tab\\\":\\\"first\\\"}\"}," +
		"{\"sequence\":2,\"type\":\"tool_result\",\"tool_call_id\":\"call-1\",\"tool_name\":\"inspect_tab\",\"status\":\"completed\",\"content\":\"{\\\"ok\\\":true}\",\"image\":{\"path\":\"images/000.png\",\"source\":\"browser\",\"browser_id\":\"browser-1\",\"target_id\":\"tab-1\",\"mime_type\":\"image/png\",\"byte_length\":7,\"width\":3,\"height\":2,\"sha256\":\"abc\",\"typed_projection\":\"screen\"}}]}\n"
	if string(got) != want {
	...
	if len(timings) != 1 {
		t.Fatalf("timings = %+v, want one entry", timings)
	}
	if timings[0].TurnIndex != 1 || timings[0].CommittedAt != "2026-08-30T12:00:00Z" || timings[0].FirstResponseAudioMS != 1250 {
	...
	if err != nil {
		t.Fatalf("second JSONL: %v", err)
	}
	if string(second) != string(got) {
		t.Fatalf("repeated JSONL changed: first=%q second=%q", got, second)
	}
}

func TestPublicServiceCorrelatesLateAndEmptyTranscripts(t *testing.T) {
	reducer := conversationlogwire.NewService(nil)
	...
	item := func(id string) {
		reducer.Observe(messages.StreamMessage{
			Type:  messages.StreamTypeInputItemAdded,
			Role:  messages.RoleUser,
			Value: messages.NewInputItemAddedValue(id),
		}, false, -1, -1)
	}
	transcriptDelta := func(text, id string) {
		reducer.Observe(messages.StreamMessage{
			Type:  messages.StreamTypeTranscriptDelta,
			Role:  messages.RoleUser,
			Value: messages.NewTranscriptDeltaValueForItem(text, id),
		}, false, -1, -1)
	}
	transcriptEnd := func(text, id string) {
		reducer.Observe(messages.StreamMessage{
			Type:  messages.StreamTypeTranscriptEnd,
			Role:  messages.RoleUser,
			Value: messages.NewTranscriptEndValueForItem(text, id),
		}, false, -1, -1)
	}
	...
	wantInputs := []string{"first turn", "second turn", ""}
...
func TestPublicServiceBoundsValuesAndDeeplySnapshotsState(t *testing.T) {
	reducer := conversationlogwire.NewService(nil)
	const exactBytes = conversationlog.MaxToolEventValueBytes
	exact := strings.Repeat("x", exactBytes)
	// The first candidate cut lands on the first byte of the final rune. The
	// reducer must retreat to a valid boundary before appending the suffix.
	...
	if bounded != wantBounded {
		t.Fatalf("split-rune content bytes=%d want=%d valid=%t hasSuffix=%t gotPrefix=%t", len(bounded), len(wantBounded), utf8.ValidString(bounded), strings.HasSuffix(bounded, conversationlog.ToolEventTruncationSuffix), strings.HasPrefix(bounded, wantSplitPrefix))
	}
	if len(bounded) > exactBytes || !utf8.ValidString(bounded) || !strings.HasSuffix(bounded, conversationlog.ToolEventTruncationSuffix) {
		t.Fatalf("bounded content invalid: bytes=%d valid=%t content suffix=%q", len(bounded), utf8.ValidString(bounded), bounded[len(bounded)-len(conversationlog.ToolEventTruncationSuffix):])
	}
	...
	if !strings.Contains(string(raw), `"content":""`) {
		t.Fatalf("empty result content was omitted: %s", raw[len(raw)-200:])
	}
	...
	snapshotReducer := conversationlogwire.NewService(nil)
	snapshotReducer.Observe(audioDelta([]byte{1}), true, 0, -1)
	snapshotReducer.Observe(messageEnd(), true, -1, -1)
	snapshotReducer.ObserveToolResult(messages.ToolCall{ID: "image", Name: "screen"}, messages.ToolCallResponse{Content: "ok"}, false, image)
	image.Path = "mutated-before-snapshot"
	first := snapshotReducer.Entries()
	...
	firstRaw, err := snapshotReducer.JSONL()
	if err != nil {
		t.Fatalf("snapshot JSONL: %v", err)
	}
	secondRaw, err := snapshotReducer.JSONL()
	if err != nil {
		t.Fatalf("repeated snapshot JSONL: %v", err)
	}
	if string(firstRaw) != string(secondRaw) || !strings.Contains(string(firstRaw), "images/original.png") {
		t.Fatalf("rendering was not stable or retained mutated input: %s", firstRaw)
	}
	second := snapshotReducer.Entries()
...
func TestPublicWireCreatesIndependentReducers(t *testing.T) {
	first := conversationlogwire.NewService(nil)
	second := conversationlogwire.NewService(nil)
	for _, reducer := range []conversationlog.Service{first, second} {
		reducer.Observe(audioDelta([]byte{1}), true, 0, -1)
		reducer.Observe(messageEnd(), true, -1, -1)
		reducer.Observe(messages.StreamMessage{
			Type:  messages.StreamTypeInputItemAdded,
			Role:  messages.RoleUser,
			Value: messages.NewInputItemAddedValue("same-item"),
		}, false, -1, -1)
	}
	...
	firstCall := messages.ToolCall{ID: "first-call", Name: "tool"}
	secondCall := messages.ToolCall{ID: "second-call", Name: "tool"}
	first.ObserveToolCall(firstCall)
	second.ObserveToolCall(secondCall)
...
func TestPublicServiceIgnoresInvalidObservationsAndOmitsTimingWithoutClock(t *testing.T) {
	reducer := conversationlogwire.NewService(nil)
...
func TestPublicServiceRetainsMissingToolIdentityAndPartialTurns(t *testing.T) {
	reducer := conversationlogwire.NewService(nil)
	// A bare boundary with no observed content is omitted.
	reducer.Observe(messageEnd(), false, -1, -1)
	if got := reducer.Entries(); got != nil {
		t.Fatalf("empty boundary entries = %+v, want nil", got)
	}

	// Missing tool identity is explicit evidence, not a fabricated ID/name.
	...
	want := "{\"turn_index\":1,\"input\":{\"text\":\"\",\"audio_bytes\":0,\"committed\":false},\"response\":{\"text\":\"partial\",\"complete\":false,\"audio_bytes\":0},\"tool_events\":[" +
		"{\"sequence\":1,\"type\":\"tool_call\",\"tool_call_id\":\"\",\"tool_name\":\"\",\"arguments\":\"\"}," +
		"{\"sequence\":2,\"type\":\"tool_result\",\"tool_call_id\":\"\",\"tool_name\":\"\",\"status\":\"completed\",\"content\":\"\"}]}\n"
	if string(raw) != want {
		t.Fatalf("partial/missing-identity JSONL = %q, want %q", raw, want)
	}
}

func TestPublicServiceConcurrentObservationAndSnapshots(t *testing.T) {
	reducer := conversationlogwire.NewService(nil)
