...
func cloneBrowserConversationResult(result BrowserConversationResult) BrowserConversationResult {
	...
	if result.InvocationObservations != nil {
		clone.InvocationObservations = make([]BrowserConversationInvocationObservation, len(result.InvocationObservations))
		for index, observation := range result.InvocationObservations {
			clone.InvocationObservations[index] = observation
			clone.InvocationObservations[index].InvocationID = cloneBrowserConversationOpaque(observation.InvocationID)
			clone.InvocationObservations[index].State = cloneBrowserConversationOpaque(observation.State)
		}
	}
