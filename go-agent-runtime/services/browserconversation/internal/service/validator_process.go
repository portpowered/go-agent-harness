...
func runBrowserConversationValidator(ctx context.Context, command []string, dir string, env []string, payload []byte, timeout time.Duration) browserConversationValidatorProcessResult {
	...
	if err := process.Start(); err != nil {
		return browserConversationValidatorProcessResult{err: errors.Join(browserconversation.ErrBrowserConversationValidatorStart, err)}
	}
	...
	case err := <-waited:
		if err != nil {
			return browserConversationValidatorProcessResult{
				...
				err: errors.Join(browserconversation.ErrBrowserConversationValidatorFailed, err),
			}
		}
	...
	case <-boundedContext.Done():
		...
		return browserConversationValidatorProcessResult{err: errors.Join(browserconversation.ErrBrowserConversationValidatorTimeout, cleanupErr)}
	}
}
