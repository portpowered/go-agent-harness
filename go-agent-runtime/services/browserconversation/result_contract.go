...
// BrowserConversationInvocationObservation records the cancellation
// publication boundary. Sequence is assigned while the run mutex is held so
// late events cannot be published ahead of their suppression accounting.
type BrowserConversationInvocationObservation struct {
	Sequence     uint64 `json:"sequence"`
	Source       string `json:"source"`
	InvocationID any    `json:"invocation_id"`
	State        any    `json:"state,omitempty"`
	Terminal     bool   `json:"terminal"`
}
