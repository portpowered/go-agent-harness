...
func (s *Storage) Save(sessionID string, msgs []messages.Message) (saveErr error) {
	if err := os.MkdirAll(s.sessionsDir, sessionDirectoryMode); err != nil {
	...
	defer func() {
		if tmpName != "" {
			if err := os.Remove(tmpName); err != nil && !os.IsNotExist(err) {
				saveErr = errors.Join(saveErr, fmt.Errorf("remove temporary session: %w", err))
			}
		}
	}()
	if _, err := tmp.Write(data); err != nil {
		return fmt.Errorf("write session %s: %w", sessionID, errors.Join(err, tmp.Close()))
	}
	if err := tmp.Close(); err != nil {
		return fmt.Errorf("write session %s: %w", sessionID, err)
	}
	if err := os.Chmod(tmpName, sessionFileMode); err != nil {
		return fmt.Errorf("write session %s: %w", sessionID, err)
	}
...
func storedContentPartsToParts(stored []storedContentPart) ([]messages.ContentPart, error) {
	...
	for _, p := range stored {
		...
		case "image":
			out = append(out, messages.ImagePart{MediaType: p.MediaType, Bytes: p.Bytes, URL: p.URL})
		case "audio":
			out = append(out, messages.AudioPart{MediaType: p.MediaType, Bytes: p.Bytes, URL: p.URL})
		case "video":
			out = append(out, messages.VideoPart{MediaType: p.MediaType, Bytes: p.Bytes, URL: p.URL})
		case "file":
			out = append(out, messages.FilePart{MediaType: p.MediaType, Bytes: p.Bytes, URL: p.URL, Name: p.Name})
		case "reasoning":
