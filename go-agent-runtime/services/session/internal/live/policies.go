...
// AllowUnadvertisedTools preserves the executor-only compatibility marker
// across the runtime deadline wrapper. New compositions keep the normal
// capability allowlist; legacy injected executors intentionally own their
// complete fixture surface.
...
// interactivePolicyToolExecutor applies the public runtime policy at the
// live session boundary. A class-specific deadline is a tool-local failure:
// return it as correlated content so ToolRunner can deliver the result and
// the provider can produce the grounded continuation that follows it.
type interactivePolicyToolExecutor struct {
	inner           messages.ToolExecutor
	scheduler       platformclock.Scheduler
...
func newInteractivePolicyToolExecutor(
	inner messages.ToolExecutor,
	scheduler platformclock.Scheduler,
	policy runtimeTools.InteractiveToolPolicy,
	timeoutOverride time.Duration,
	...
	var snapshot runtimeTools.InteractiveToolPolicy
	if policy != nil {
		snapshot = policy.Clone()
	}
	if snapshot == nil && timeoutOverride <= 0 {
		return inner
	}
	return interactivePolicyToolExecutor{
		inner:           inner,
		scheduler:       scheduler,
		policy:          snapshot,
...
func (e interactivePolicyToolExecutor) Execute(ctx context.Context, call messages.ToolCall) (messages.ToolCallResponse, error) {
	if ctx == nil {
	...
	if errors.Is(toolCtx.Err(), context.DeadlineExceeded) && ctx.Err() == nil {
		return messages.ToolCallResponse{
			...
			Name:       call.Name,
			Content: fmt.Sprintf(
				"tool %q failed (classification=%s): %s after %s",
				call.Name,
				runtimeTools.InteractiveToolTimeoutClassification,
				session.ErrLiveToolExecutionTimeout,
				timeout,
			),
		}, nil
	}
...
// AllowUnadvertisedTools carries the explicit executor-replacement contract
// through policy enforcement. Without this forwarding method, resolving a
// policy would accidentally turn compatibility fixtures into an empty
// capability surface before restrictToolExecutor sees them.
func (e interactivePolicyToolExecutor) AllowUnadvertisedTools() bool {
	replacement, ok := e.inner.(interface{ AllowUnadvertisedTools() bool })
	return ok && replacement.AllowUnadvertisedTools()
}
