...
func TestTerminalDrainOrderedBoundaryTrace(t *testing.T) {
	...
	for read < len(samples) {
		frame, readErr := provider.media.Endpoints().Inbound.ReadFrame(readContext)
		if readErr != nil {
			t.Fatalf("render input frame after admission (%d samples): %v", read, readErr)
		}
	...
	forwarded, readErr := terminal.receive.ReadContext(readContext)
	if readErr != nil {
		t.Fatalf("read forwarded response terminal: %v", readErr)
	}
	...
	trace.record("response_terminal", len(samples), len(samples))
	if err := connected.Close(); err != nil {
	...
	for index, want := range wantBoundaries {
		if events[index].Sequence != index+1 || events[index].Boundary != want || events[index].TimingDomain != "monotonic-process" {
			t.Fatalf("ordered boundary event %d = %#v, want sequence/boundary %d/%s", index, events[index], index+1, want)
		}
		if events[index].SampleStart < 0 || events[index].SampleEnd < events[index].SampleStart || events[index].SampleEnd > terminalDrainBoundarySamples {
			t.Fatalf("ordered boundary event %d has invalid sample range: %#v", index, events[index])
		}
	}
	...
	t.Logf("C127_ORDERED_BOUNDARY_TRACE events=%s", encoded)
}
