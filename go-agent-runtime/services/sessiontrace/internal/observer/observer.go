...
type ScheduledAudioDispatchPolicy = sessiontrace.ScheduledAudioDispatchPolicy
...
// observerState derives metrics observations and diagnostic records
...
type observerState struct {
	...
	sink               sessiontrace.DiagnosticSink
	terminal           sessiontrace.TerminalService
	recorder           metrics.Recorder
	productionSink     *metrics.InMemorySink
	streamObserver     sessiontrace.StreamObserver
	...
	admittedTurnObserver sessiontrace.StreamObserver
	...
	livenessTimer          sessiontrace.LivenessTimer
...
func (o *observerState) markRoomBoundCancellation() {
	if o != nil {
		o.roomBoundCancellation = true
	}
}
func (o *observerState) notifyTerminalObservation(observation sessionTerminalObservation) bool {
	if o == nil {
		return false
	}
...
func (o *observerState) notifyFailureObservation(observation sessionTerminalObservation) bool {
...
func newObserverState(sink sessiontrace.DiagnosticSink, recorder metrics.Recorder, provider, model string) *observerState {
	...
	return &observerState{
		lifecycle:             lifecycleService.New(lifecycle.Options{}),
		sink:                  sink,
		recorder:              recorder,
		...
		livenessClock:         platformSessionLivenessClock{source: platformclock.Real{}},
		livenessWakeCh:        make(chan struct{}, 1),
	}
}

func (o *observerState) lockProviderBoundary() func() {
	if o == nil {
		return func() {}
	}
...
func (o *observerState) scheduleAudioInputs(inputs []ScheduledAudioInput) {
	if o == nil {
		return
	}
