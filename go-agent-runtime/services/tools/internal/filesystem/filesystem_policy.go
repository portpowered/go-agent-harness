...
// reusable runtime code never discovers the process current working directory.
// Relative additional roots are resolved against that captured primary root,
// not against a later process cwd.
func ResolveFilesystemPolicy(workdir string, additionalRoots ...string) (*FilesystemPolicy, error) {
	if strings.TrimSpace(workdir) == "" {
		return nil, fmt.Errorf("%w: workdir must be supplied by the host boundary", ErrInvalidFilesystemRoot)
	}
...
func NewFilesystemPolicy(primaryRoot string, additionalRoots ...string) (*FilesystemPolicy, error) {
	...
	protectedRoots := platformProtectedReadRoots()
	for _, scopeRoot := range append([]string{primary}, roots...) {
		protectedRoots = append(protectedRoots, credentialReadRoots(scopeRoot)...)
	}
	return &FilesystemPolicy{
		primaryRoot:        primary,
		additionalRoots:    roots,
		protectedReadRoots: normalizeProtectedReadRoots(protectedRoots),
	}, nil
}
...
// platformProtectedReadRoots lists operating-system data and common user
// credential stores that filesystem tools must never read through a widened
// policy. The list is path-based and independent of file permissions: a
// readable secret is still a protected secret.
func platformProtectedReadRoots() []string {
	if runtime.GOOS == windowsPlatform {
		return windowsProtectedReadRoots()
	}
	return unixProtectedReadRoots()
}

func unixProtectedReadRoots() []string {
	roots := []string{
		"/etc",
		"/private/etc",
		"/proc",
...
func windowsProtectedReadRoots() []string {
	roots := []string{}
...
func credentialReadRoots(scopeRoot string) []string {
	return []string{
		filepath.Join(scopeRoot, ".ssh"),
		filepath.Join(scopeRoot, ".aws"),
		filepath.Join(scopeRoot, ".azure"),
		filepath.Join(scopeRoot, ".docker"),
		filepath.Join(scopeRoot, ".gnupg"),
		filepath.Join(scopeRoot, ".kube"),
		filepath.Join(scopeRoot, ".config", "gcloud"),
		filepath.Join(scopeRoot, ".config", "gh"),
		filepath.Join(scopeRoot, ".config", "hub"),
		filepath.Join(scopeRoot, ".local", "share", "keyrings"),
		filepath.Join(scopeRoot, ".git-credentials"),
		filepath.Join(scopeRoot, ".netrc"),
		filepath.Join(scopeRoot, ".npmrc"),
		filepath.Join(scopeRoot, ".pypirc"),
		filepath.Join(scopeRoot, "Library", "Keychains"),
		filepath.Join(scopeRoot, "AppData", "Local", "Microsoft", "Credentials"),
		filepath.Join(scopeRoot, "AppData", "Local", "Microsoft", "IdentityCache"),
		filepath.Join(scopeRoot, "AppData", "Roaming", "Microsoft", "Credentials"),
		filepath.Join(scopeRoot, "AppData", "Roaming", "gcloud"),
	}
}
