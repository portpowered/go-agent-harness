...
func TestReadImageTool_ReturnsExactlyOneRichImagePart(t *testing.T) {
	...
	if result.MIMEType != imagePNGMediaType || result.ByteLength != len(wantBytes) {
	...
	if part.MediaType != imagePNGMediaType || !bytes.Equal(part.Bytes, wantBytes) {
...
func TestReadImageTool_SuccessEnvelopeSizeIsIndependentOfImageSize(t *testing.T) {
	...
	for name, imageBytes := range map[string][]byte{"small": small, "multi-megabyte": large} {
		t.Run(name, func(t *testing.T) {
			assertImageEnvelopeSize(t, name, imageBytes)
		})
	}
}
...
func assertImageEnvelopeSize(t *testing.T, name string, imageBytes []byte) {
	...
	assertCompactImageEnvelope(t, envelope)
	var result ReadImageResult
	if err := json.Unmarshal([]byte(envelope), &result); err != nil {
		t.Fatalf("decode result envelope: %v", err)
	}
	digest := sha256.Sum256(imageBytes)
	if result.Version != ReadImageResultVersion || result.Status != ReadImageResultStatusSuccess || result.MIMEType != imagePNGMediaType || result.ByteLength != len(imageBytes) || result.SHA256 != hex.EncodeToString(digest[:]) || result.TypedProjection != ReadImageResultTypedProjectionInputImage {
...
func assertCompactImageEnvelope(t *testing.T, envelope string) {
	t.Helper()
...
func TestReadImageTool_RejectsInvalidPreparerContentWithVersionedError(t *testing.T) {
	for name, part := range map[string]messages.ImagePart{
		"empty":      {MediaType: imagePNGMediaType},
		"corrupt":    {Bytes: []byte("not an image"), MediaType: imagePNGMediaType},
