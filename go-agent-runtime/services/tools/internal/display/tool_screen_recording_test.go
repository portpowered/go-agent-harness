...
func TestScreenRecordingLimitsAndDefaults(t *testing.T) {
	...
	if !ok {
		t.Fatalf("recording schema properties have type %T", tool.Parameters()["properties"])
	}
	duration, ok := params["duration"].(map[string]any)
	if !ok {
		t.Fatalf("duration schema has type %T", params["duration"])
	}
	fps, ok := params["fps"].(map[string]any)
	if !ok {
		t.Fatalf("fps schema has type %T", params["fps"])
	}
