...
func (p *fakeShellProcess) Start() error {
	...
	if p.stdout != "" {
		writeShellTestOutput(p.stdoutWriter, p.stdout)
	}
	if p.stderr != "" {
		writeShellTestOutput(p.stderrWriter, p.stderr)
	}
	close(p.started)
	return nil
}

func writeShellTestOutput(writer io.Writer, output string) {
...
type shellProcessOutcome struct {
	name          string
...
func TestExecTool_S4InjectedProcessOutcomes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			assertShellProcessOutcome(t, test)
		})
	}
}

func assertShellProcessOutcome(t *testing.T, test shellProcessOutcome) {
	t.Helper()
	tool := NewExecTool("", false)
	injectFakeShellProcess(tool, test.process)
	tool.SetTimeout(test.timeout)
	ctx, cancel := shellOutcomeContext(test.cancelMidRun)
	defer cancel()
	got, err := executeShellOutcome(t, tool, ctx, test)
	assertShellOutcomeError(t, err, test)
	assertShellOutcomeMessages(t, got, test)
	if gotKillCount := test.process.killCount.Load(); gotKillCount != test.wantKillCount {
		t.Fatalf("kill count = %d, want %d", gotKillCount, test.wantKillCount)
	}
}

func shellOutcomeContext(cancelMidRun bool) (context.Context, context.CancelFunc) {
	if !cancelMidRun {
...
func executeShellOutcome(t *testing.T, tool *ExecTool, ctx context.Context, test shellProcessOutcome) ([]messages.Message, error) {
	t.Helper()
	if !test.cancelMidRun {
		return tool.Execute(ctx, map[string]any{"command": "synthetic"})
	}
...
func assertShellOutcomeError(t *testing.T, err error, test shellProcessOutcome) {
	t.Helper()
	if test.wantError != "" {
...
func assertShellOutcomeMessages(t *testing.T, got []messages.Message, test shellProcessOutcome) {
	t.Helper()
	if test.wantOutput == "" {
		if len(got) != 0 {
...
func TestExecTool_ValidationGuardsAndConfiguration(t *testing.T) {
	...
	assertShellRequiredAndDangerous(t, ctx)
	workspace := t.TempDir()
	assertShellWorkspaceRestrictions(t, ctx, workspace)
	assertShellAllowlistAndOutput(t, ctx)
	assertShellDenyConfiguration(t, ctx)
	assertShellDisabledDeny(t, ctx)
}

func assertShellRequiredAndDangerous(t *testing.T, ctx context.Context) {
	t.Helper()
	tool := NewExecTool("", false)
	for _, args := range []map[string]any{{}, {"command": 123}} {
		requireShellError(t, tool, ctx, args, "command is required", "command-required")
	}
	requireShellError(t, tool, ctx, map[string]any{"command": "rm -rf target"}, "dangerous pattern", "dangerous command")
}

func assertShellWorkspaceRestrictions(t *testing.T, ctx context.Context, workspace string) {
	t.Helper()
	restricted := NewExecTool(workspace, true)
	requireShellError(t, restricted, ctx, map[string]any{"command": "echo ../outside"}, "path traversal", "traversal command")
	requireShellError(t, restricted, ctx, map[string]any{
		"command":     "safe",
		"working_dir": filepath.Join(workspace, ".."),
	}, "outside the workspace", "outside working directory")
	outsidePath := filepath.Join(workspace, "..", "outside")
	requireShellError(t, restricted, ctx, map[string]any{"command": "echo " + outsidePath}, "outside working dir", "outside command path")
}

func assertShellAllowlistAndOutput(t *testing.T, ctx context.Context) {
	t.Helper()
	...
	assertShellEmptyOutput(t, ctx)
}

func assertShellEmptyOutput(t *testing.T, ctx context.Context) {
	t.Helper()
...
func assertShellDenyConfiguration(t *testing.T, ctx context.Context) {
	t.Helper()
	custom := public.ExecPolicy{Configured: true, EnableDenyPatterns: true, CustomDenyPatterns: []string{"forbidden", "["}}
	customTool := NewExecToolWithPolicyAndDiagnosticWriter("", false, custom, nil)
	requireShellError(t, customTool, ctx, map[string]any{"command": "forbidden"}, "dangerous pattern", "custom deny")
	defaults := public.ExecPolicy{Configured: true, EnableDenyPatterns: true}
	defaultTool := NewExecToolWithPolicyAndDiagnosticWriter("", false, defaults, nil)
	requireShellError(t, defaultTool, ctx, map[string]any{"command": "rm -rf target"}, "dangerous pattern", "default deny")
}

func assertShellDisabledDeny(t *testing.T, ctx context.Context) {
	t.Helper()
	disabled := public.ExecPolicy{Configured: true}
	disabledProcess := newFakeShellProcess("deny disabled", "", nil, nil, false)
	disabledTool := NewExecToolWithPolicyAndDiagnosticWriter("", false, disabled, nil)
...
func requireShellError(t *testing.T, tool *ExecTool, ctx context.Context, args map[string]any, want, label string) {
	t.Helper()
	if _, err := tool.Execute(ctx, args); err == nil || !strings.Contains(err.Error(), want) {
...
func TestExecTool_DisabledDenyWarningDescribesShellPolicyOnly(t *testing.T) {
	...
	tool := newExecToolWithDiagnosticWriter("", false, public.ExecPolicy{Configured: true}, &diagnostics)
	if tool == nil {
