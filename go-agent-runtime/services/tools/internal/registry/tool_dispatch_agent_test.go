...
func TestDispatchAgentTool_RegistrableInRegistry(t *testing.T) {
	inf := &dispatchMockInferencer{response: "done"}
	registry := NewToolRegistry()
	if err := registry.Register(NewDispatchAgentTool(inf, registry)); err != nil {
		t.Fatalf("register dispatch_agent: %v", err)
	}

	tool, ok := registry.Get(dispatchAgentToolID)
	if !ok {
		t.Fatal("dispatch_agent should be retrievable after registration")
	}
	if tool.Name() != dispatchAgentToolID {
		t.Errorf("expected name %q, got %q", dispatchAgentToolID, tool.Name())
	}
}
