...
func (t *DispatchAgentTool) Name() string { return dispatchAgentToolID }
...
func (t *DispatchAgentTool) Execute(ctx context.Context, args map[string]any) ([]messages.Message, error) {
	...
	if !ok || systemPrompt == "" {
		return core.ErrorAsToolMessage(fmt.Errorf("system_prompt is required"))
	}
	task, ok := args["task"].(string)
	if !ok || task == "" {
		return core.ErrorAsToolMessage(fmt.Errorf("task is required"))
	}
	...
	if err != nil {
		return core.ErrorAsToolMessage(fmt.Errorf("failed to create child agent: %w", err))
	}
	...
	if err != nil {
		return core.ErrorAsToolMessage(fmt.Errorf("child agent error: %w", err))
	}
...
func (t *DispatchAgentTool) buildChildRegistry(toolNames []string) *ToolRegistry {
	...
	child := &ToolRegistry{
		...
		diagnosticWriter: registryDiagnosticWriter(t.registry),
	}
	if len(toolNames) == 0 {
