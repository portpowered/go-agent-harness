...
func validateRoomReplayArtifacts(root string, refs []roomReplayArtifactRef, metadata map[string]roomReplayArtifactRef) ([]RoomReplayArtifact, map[string]RoomReplayArtifact, error) {
	...
	for _, ref := range refs {
		...
		if *declared.Size > roomReplayMaxArtifactBytes {
			return nil, nil, newRoomReplayBundleError(RoomReplayBundleMismatch, ref.Field, normalized, fmt.Sprintf("size at most %d", roomReplayMaxArtifactBytes), fmt.Sprintf("size %d", *declared.Size), ErrInvalidRoomReplayBundle)
		}
		...
		if actualSize > roomReplayMaxArtifactBytes {
			return nil, nil, newRoomReplayBundleError(RoomReplayBundleMismatch, ref.Field, normalized, fmt.Sprintf("size at most %d", roomReplayMaxArtifactBytes), fmt.Sprintf("size %d", actualSize), ErrInvalidRoomReplayBundle)
		}
...
func loadRoomReplayTimeline(artifact RoomReplayArtifact, participants map[string]struct{}, declared map[string]RoomReplayArtifact, clockBase, startedAt, endedAt time.Time) ([]RoomReplayTimelineEvent, error) {
	...
	for lineNumber := 1; scanner.Scan(); lineNumber++ {
		...
		if totalBytes > roomReplayMaxTimelineBytes {
			return nil, newRoomReplayBundleError(RoomReplayBundleMismatch, "room_timeline", artifact.Path, fmt.Sprintf("timeline size at most %d bytes", roomReplayMaxTimelineBytes), fmt.Sprintf("more than %d bytes", roomReplayMaxTimelineBytes), ErrInvalidRoomReplayBundle)
		}
		if strings.TrimSpace(string(line)) == "" {
			continue
		}
		if int64(len(result))+1 > roomReplayMaxTimelineRecords {
			return nil, newRoomReplayBundleError(RoomReplayBundleMismatch, "room_timeline", artifact.Path, fmt.Sprintf("at most %d records", roomReplayMaxTimelineRecords), fmt.Sprintf("more than %d records", roomReplayMaxTimelineRecords), ErrInvalidRoomReplayBundle)
		}
