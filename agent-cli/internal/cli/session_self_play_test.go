...
func TestSessionSelfPlayCommandParsesBoundedRunOptions(t *testing.T) {
	...
	subject := NewSessionSelfPlayCommand(globalFlags)

	var got services.SelfPlayRunOptions
	subject.SetRunner(func(_ context.Context, _ io.Writer, opts services.SelfPlayRunOptions) error {
		got = opts
		return nil
	})
...
func TestSessionSelfPlayCommandHelpDocumentsFixedPhaseOneContract(t *testing.T) {
	cmd := NewSessionSelfPlayCommand(flags.NewGlobalFlags()).Generate()
