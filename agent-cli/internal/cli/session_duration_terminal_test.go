...
func TestSessionCommandMaxDurationRejectsInvalidPartialArtifact(t *testing.T) {
	...
	recordingDir := filepath.Join(artifactRoot, "recording")
	root := newTestRootCommandWithProbeFleetCommand(NewProbeFleetCommand(), newCLIDurationInferencer(cliDurationInvalidAudioEvents()))
	var stdout, stderr bytes.Buffer
	root.SetOut(&stdout)
	root.SetErr(&stderr)
	root.SetArgs([]string{
	...
	if !strings.Contains(err.Error(), "odd PCM16 byte count") {
...
func assertDurationSidecarArtifacts(t *testing.T, recordPath, wantText string, wantSamples []int16) {
	...
	for _, line := range bytes.Split(bytes.TrimSpace(data), []byte("\n")) {
		...
		if event.Type == messages.StreamTypeSessionClose {
			...
			sawMaxDuration = value.TerminalReason == services.SessionMaxDurationReason
		}
	}
...
func assertDurationRecordingBundle(t *testing.T, recordingDir string, wantAudio []byte) {
	...
	if manifest.Terminal == nil || manifest.Terminal.TerminalReason != services.SessionMaxDurationReason || manifest.Terminal.OutputState != messages.TerminalOutputPartial {
