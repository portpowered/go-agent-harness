...
func resampleTest6PCM(t *testing.T, pcm []byte) []int16 {
	...
	converted, err := wavio.Resample(samples, wavio.Rate24kHz, audio.SampleRate)
	if err != nil {
		t.Fatalf("resample test6 PCM: %v", err)
	}
	return converted
}
...
func (r *test6PlaybackRegistry) List() ([]audio.Device, error) {
	return []audio.Device{r.device.metadata}, nil
}
func (r *test6PlaybackRegistry) Default(direction audio.Direction) (audio.Device, error) {
	if direction != audio.DirectionOutput {
		return audio.Device{}, audio.NewNoDefaultDeviceError(direction)
	}
	return r.device.metadata, nil
}
func (r *test6PlaybackRegistry) Open(id audio.DeviceID) (audio.OpenedDevice, error) {
...
func newTest6RecordingPlaybackDevice(t *testing.T) *test6RecordingPlaybackDevice {
	t.Helper()
	metadata, err := audio.NewDevice("test6", "output", "test6 recording playback", audio.DirectionOutput)
	if err != nil {
		t.Fatal(err)
	}
...
var _ audio.DeviceRegistry = (*test6PlaybackRegistry)(nil)
