...
func TestSessionAmbiguousCubeConversationRequiresChoiceBeforePageWork(t *testing.T) {
	...
	answer := make(chan services.ScheduledAudioInput, 1)
	var output strings.Builder
	sessionCtx, cancelSession := context.WithCancel(ctx)
	runErr := make(chan error, 1)
	runComplete := make(chan struct{})
	go func() {
		err := services.RunSessionWithInstructions(sessionCtx, &output, services.SessionRunOptions{
			Provider:               config.ProviderGrok,
			Model:                  "ambiguous-session",
			APIKey:                 "unused",
			ConfigDir:              t.TempDir(),
	...
	answer <- services.ScheduledAudioInput{PCM: []byte{1, 2, 3}, EndOfTurn: true}
