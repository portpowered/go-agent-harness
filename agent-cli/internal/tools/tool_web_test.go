...
func TestNewToolRegistry_IncludesWebTools(t *testing.T) {
	registry := NewToolRegistry()

	_, hasFetch := registry.Get("web_fetch")
	if !hasFetch {
		t.Error("default registry should include web_fetch tool")
	}

	_, hasSearch := registry.Get("web_search")
	if !hasSearch {
		t.Error("default registry should include web_search tool")
	}
}
