...
func TestStatefulBrokerRejectsPageInputWithSelectedSchemaAndStableIssues(t *testing.T) {
	...
	for _, testCase := range inputCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			classified := requireInvalidToolInput(t, err)
			...
			assertInputIssuePresent(t, classified, testCase.wantPath, testCase.wantCode)
			if testCase.noEcho != "" {
...
func requireInvalidToolInput(t *testing.T, err error) *webmcp.ClassifiedError {
	t.Helper()
	if err == nil {
...
func assertInputIssuePresent(t *testing.T, classified *webmcp.ClassifiedError, wantPath, wantCode string) {
	t.Helper()
	...
	if !found {
		t.Fatalf("issues = %#v, want %s/%s", issues, wantPath, wantCode)
	}
}

func assertDetailsDoNotEcho(t *testing.T, classified *webmcp.ClassifiedError, noEcho string) {
	t.Helper()
	encoded, marshalErr := json.Marshal(classified.Details)
	if marshalErr != nil {
		t.Fatalf("marshal safe details: %v", marshalErr)
	}
	if strings.Contains(string(encoded), noEcho) {
		t.Fatalf("details echoed raw input value %q: %s", noEcho, encoded)
	}
}
