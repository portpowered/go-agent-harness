...
func (e *focusExecutor) Execute(ctx context.Context, method string, params, result any) error {
	...
	e.mu.Lock()
	defer e.mu.Unlock()
	e.enabled = append(e.enabled, params.(*emulation.SetFocusEmulationEnabledParams).Enabled)
	if e.failNextEnable && params.(*emulation.SetFocusEmulationEnabledParams).Enabled {
