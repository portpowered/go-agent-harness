...
func generateAgentsMD(workspaceDir string, toolDefs []messages.ToolDefinition) string {
	...
	// --- Configuration ---
	writeAgentsMDConfiguration(&sb, workspaceDir)

	// --- CLI Commands ---
	writeAgentsMDCommands(&sb)
...
// writeAgentsMDConfiguration writes the configuration file and environment
// variable reference section.
func writeAgentsMDConfiguration(sb *strings.Builder, workspaceDir string) {
...
// writeAgentsMDCommands writes the CLI command reference section.
func writeAgentsMDCommands(sb *strings.Builder) {
