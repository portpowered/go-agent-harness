...
func TestRunSessionHeadphoneShapedPlaybackPreservesIndependentSpeech(t *testing.T) {
	...
	userFeed, err := audio.NewDeviceSink(registry, rtcRoundtripMicFeedID)
	if err != nil {
		t.Fatalf("open virtual microphone feeder: %v", err)
	}
	speakerObserve, err := audio.NewDeviceSource(registry, rtcRoundtripSpeakerID)
	if err != nil {
		_ = userFeed.Close()
	...
	providerFrames := make(chan rtc.PCMFrame, feedbackBypassFrameCount+1)
	media := rtc.NewSessionMedia(func(ctx context.Context, frame rtc.PCMFrame) error {
...
func TestRunSessionReplayBypassesPairedDeviceFeedbackController(t *testing.T) {
	...
	media := rtc.NewSessionMedia(func(ctx context.Context, frame rtc.PCMFrame) error {
		select {
		case providerFrames <- rtc.PCMFrame{Samples: append([]int16(nil), frame.Samples...)}:
			return nil
		case <-ctx.Done():
			return ctx.Err()
		}
...
func newFeedbackBypassSessionInferencer(media *rtc.SessionMedia) *feedbackBypassSessionInferencer {
