...
type deviceProbeRuntimeOptions = serviceDevices.DeviceProbeRequest
...
type deviceProbeInputPlan = serviceDevices.DeviceProbeInputPlan

func runDeviceProbeScenario(ctx context.Context, scenario probe.Scenario, availability devicegw.DeviceProbeAvailability, registry devicegw.DeviceRegistry, opts deviceProbeRuntimeOptions) (observation probe.ObservationSnapshot, runErr error) {
	if ctx == nil {
	...
	source, err := devicegw.NewDeviceSource(registry, inputDevice.ID)
	if err != nil {
	...
	sink, err := devicegw.NewDeviceSink(registry, outputDevice.ID)
	if err != nil {
	...
	if inferencer == nil {
		var model string
		inferencer, model, err = agentruntime.NewLiveSessionInferencer(agentruntime.SessionRunOptions{
...
func scenarioDeviceProbeInput(scenario probe.Scenario) (deviceProbeInputPlan, error) {
	return serviceDevices.ProbeInputPlan(scenario)
}
