...
func recordRoomTimelineEvent(evidence roomevidence.Recorder, participantID string, msg messages.StreamMessage) {
	if evidence == nil {
		return
	}
	switch msg.Type {
	case messages.StreamTypeMessageStart:
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "response_start", ParticipantID: participantID, Fields: map[string]string{"response_id": msg.ResponseID}}))
	case messages.StreamTypeMessageEnd:
		...
		if value, ok := msg.Value.(*messages.MessageEndValue); ok && value != nil {
			...
			observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "response_end", ParticipantID: participantID, Fields: fields}))
			if value.TerminalReason == messages.TerminalReasonCancellation {
				...
				observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "barge_in_cancel_acked", ParticipantID: participantID, Fields: map[string]string{"response_id": msg.ResponseID}}))
			}
			return
		}
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "response_end", ParticipantID: participantID, Fields: fields}))
	case messages.StreamTypeError:
		...
		if value.Classification == providers.ErrorClassResponseCancelNotActive {
			...
			observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "barge_in_cancel_failed", ParticipantID: participantID, Fields: fields}))
			return
		}
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationProviderError, ParticipantID: participantID, Fields: fields}))
	case messages.StreamTypeToolCallStart:
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "tool_call_start", ParticipantID: participantID, Fields: map[string]string{"tool_call_id": msg.ToolCallId}}))
	case messages.StreamTypeToolCallEnd:
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "tool_call_end", ParticipantID: participantID, Fields: map[string]string{"tool_call_id": msg.ToolCallId}}))
	}
}
...
func runRoomParticipant(
	...
	runtime.diagnosticSink = combineDiagnosticSinks(roomParticipantDiagnosticSinks(runtime.plan, opts, evidence)...)
	...
	if !roomParticipantIsHuman(runtime.plan) {
		...
		observer.livenessObserver = func(err error) {
			...
			if classification != "" && evidence != nil {
				observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "live_liveness_fault", ParticipantID: participantID, Fields: map[string]string{"reason": classification}}))
			}
		}
		...
		observer.streamObserver = func(msg messages.StreamMessage) {
			observeRoomParticipantStream(coordinator, runtime, opts, evidence, msg)
		}
		observer.admittedTurnObserver = func(messages.StreamMessage) {
			...
			if evidence != nil {
				observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationTimeline, Event: "turn_completed", ParticipantID: participantID, Fields: map[string]string{"turn_index": strconv.Itoa(turns)}}))
			}
		}
	}
	...
	go func() {
		...
		if roomParticipantIsHuman(runtime.plan) {
			pumpRoomHumanOutput(roomCtx, coordinator, runtime, startGate, evidence, secrets)
			return
		}
	...
	if roomParticipantIsHuman(runtime.plan) {
		...
		runErr := runRoomHumanCapture(roomCtx, coordinator, runtime, startGate, evidence, opts, secrets)
	...
	diagnosticSinks := roomParticipantDiagnosticSinks(runtime.plan, opts, evidence)
	...
	observer.failureObserver = roomParticipantFailureHandler{runtime: runtime, evidence: evidence, coordinator: coordinator}.observe
	observer.turnAdmission = func(msg messages.StreamMessage) bool {
		value, ok := msg.Value.(*messages.MessageEndValue)
		if !ok || value == nil || value.TerminalReason == "" {
	...
	if flusher, ok := runtime.plan.inferencer.(SessionInferencerCaptureFlusher); ok {
		if flushErr := flusher.FlushCapture(); flushErr != nil && evidence != nil {
			observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationError, ParticipantID: participantID, Artifact: evidence.Artifacts(participantID).Capture, Err: flushErr}))
		}
	}
	if closeErr := closeRoomParticipantCapability(runtime.plan); closeErr != nil {
...
func roomParticipantDiagnosticSinks(
	...
	if evidence != nil {
		diagnosticSinks = append(diagnosticSinks, roomParticipantEvidenceDiagnosticSink{evidence: evidence, participantID: plan.manifest.ID})
	}
...
type roomParticipantEvidenceDiagnosticSink struct {
	evidence      roomevidence.Recorder
	participantID string
}

func (s roomParticipantEvidenceDiagnosticSink) RecordSessionDiagnostic(record SessionDiagnosticRecord) {
	if s.evidence == nil {
		return
	}
	observeRoomEvidenceResult(s.evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationDiagnostic, ParticipantID: s.participantID, Diagnostic: roomevidence.DiagnosticRecord{Event: record.Event, Fields: record.Fields}}))
}
...
func observeRoomParticipantStream(
	...
	if evidence != nil && msg.Type == messages.StreamTypeVADSpeechStopped {
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationSpeechStopped, ParticipantID: plan.manifest.ID}))
	}
	...
	recordParticipantDelta := func() {
		...
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationDelta, ParticipantID: plan.manifest.ID, StreamMessage: msg}))
	}
	...
	if msg.Type == messages.StreamTypeAudioEnd && assistantAudioDelta(msg) && evidence != nil {
		...
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationCloseSentSpeechSegment, ParticipantID: plan.manifest.ID}))
	}
	...
	if evidence != nil {
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationSpeakerAudio, ParticipantID: plan.manifest.ID, TargetIDs: targetIDs, PCM: pcm}))
		if len(pcm) > 0 {
			observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationProviderAudio, ParticipantID: plan.manifest.ID, RelatedID: msg.ResponseID}))
		}
	}
	if evidence != nil {
		...
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationSentStream, ParticipantID: plan.manifest.ID, PCM: pcm}))
	}
	...
	if !plan.replay {
		for _, target := range targets {
			...
			if evidence != nil {
				observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationPeerAudio, ParticipantID: plan.manifest.ID, RelatedID: target.plan.manifest.ID, PCM: pcm}))
			}
	...
	if evidence != nil {
		...
		observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationParticipantAudio, ParticipantID: plan.manifest.ID, PCM: pcm}))
	}
}
	...
	for {
		...
		if err := sendAudioInput(admissionCtx, providerFrame, policy); err != nil {
			...
			if evidence != nil && audio.PCM16HasSignal(frame) {
				observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationAudioDropped, ParticipantID: runtime.plan.manifest.ID, Artifact: err.Error(), DroppedBytes: len(frame)}))
			}
		...
		if evidence != nil {
			...
			observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationReceivedParticipantAudio, ParticipantID: runtime.plan.manifest.ID, PCM: providerFrame}))
		}
		if observer != nil {
...
func runRoomHumanCapture(
	...
	for {
		...
		if evidence != nil {
			...
			observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationSentAudio, ParticipantID: participantID, PCM: pcm}))
		}
...
func pumpRoomHumanOutput(ctx context.Context, coordinator *roomCoordinator, runtime *roomParticipantRuntime, startGate <-chan struct{}, evidence roomevidence.Recorder, secrets []string) {
	...
	for {
		...
		if evidence != nil {
			observeRoomEvidenceResult(evidence.Observe(roomevidence.Observation{Kind: roomevidence.ObservationReceivedParticipantAudio, ParticipantID: runtime.plan.manifest.ID, PCM: frame}))
		}
	}
}
