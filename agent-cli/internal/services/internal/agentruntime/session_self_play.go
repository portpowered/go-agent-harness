...
const (
	...
	// command contract.
	SelfPlayDefaultProvider = serviceSelfPlay.SelfPlayDefaultProvider
	...
	SelfPlayDefaultModel = serviceSelfPlay.SelfPlayDefaultModel
	// SelfPlayDefaultMaxDuration bounds an invocation that omits the flag.
	SelfPlayDefaultMaxDuration = serviceSelfPlay.SelfPlayDefaultMaxDuration
	// SelfPlayDefaultTurnTarget gives the manual smoke path the three-turn
	// target required by the Phase 1 proof.
	SelfPlayDefaultTurnTarget = serviceSelfPlay.SelfPlayDefaultTurnTarget
	...
	SelfPlayCustomerPersona  = serviceSelfPlay.SelfPlayCustomerPersona
	SelfPlayAssistantPersona = serviceSelfPlay.SelfPlayAssistantPersona
	...
	SelfPlayOpeningSeed = serviceSelfPlay.SelfPlayOpeningSeed
)
...
type SelfPlayRunOptions struct {
	...
	// clock is installed by the service constructor so both self-play sessions
	// use the application timing domain.
...
func defaultSelfPlaySessionFactory(opts SessionRunOptions, instructions string) (messages.SessionInferencer, error) {
	if opts.runtimeFactory.configured() {
		factory := sessionRuntimeFactoryWithInstructions(opts.runtimeFactory, instructions)
		...
		inferencer, _, err := factory.newBareLiveSessionInferencer(opts)
...
func runSelfPlayConversation(ctx context.Context, opts SelfPlayRunOptions, customer messages.SessionInferencer, assistant messages.SessionInferencer) (SelfPlayResult, error) {
	...
	evidence, err := newSelfPlayEvidence(opts.OutputDir, opts, selfPlayNow(opts))
	if err != nil {
...
func selfPlayNow(opts SelfPlayRunOptions) time.Time {
	...
	return platformclock.Real{}.Now().UTC()
}
