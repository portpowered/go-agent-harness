...
type sessionProgressObserver struct {
	...
	// lifecycleGate serializes provider-facing send acknowledgement with inbound
	// lifecycle observation. A provider may make a response visible immediately
	// from Send, before SendWithOutcome returns to record the accepted result or
	// continuation request; keeping that boundary ordered prevents the next
	// response from being reduced against a stale continuation state.
