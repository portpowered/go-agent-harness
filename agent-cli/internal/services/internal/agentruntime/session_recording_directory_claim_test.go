...
func TestRunSessionWithRecordingDirectoryConcurrentClaimHasOneProviderConnection(t *testing.T) {
	...
	for range 2 {
		...
		go func() {
			defer wait.Done()
			<-start
			results <- runSessionWithRecordingDirectoryForTest(context.Background(), io.Discard, SessionRunOptions{ModelCatalog: testModelCatalog(), AudioService: newTestAudioIOService(),
