...
func TestRTCDeviceBoundSessionAcceptedCancelDiscardsQueuedPlayback(t *testing.T) {
	...
	observe, err := devicegw.NewDeviceSource(registry, "virtual:input")
	if err != nil {
	...
	if err := binding.Sink.Pump(context.Background(), &recordingRTCInboundMedia{
		frames: []audio.PCMFrame{{Samples: queued}},
	}); err != nil {
	...
	if err := binding.Sink.WritePlayback(context.Background(), next); err != nil {
...
func TestRTCDeviceBoundSessionRejectedCancelDoesNotDiscardPlayback(t *testing.T) {
	...
	if err := binding.Sink.WritePlayback(context.Background(), queued); err != nil {
		t.Fatalf("queue provider playback: %v", err)
	}
...
func TestRTCDeviceSinkDiscardUnblocksPacedProviderBurst(t *testing.T) {
	...
	frames := make([]audio.PCMFrame, high/audio.FrameSize+2)
	for index := range frames {
		frames[index] = audio.PCMFrame{Samples: cancelPlaybackFrame(int16(100 + index))}
	}
...
func TestRTCDeviceSinkInterruptionUsesActuallyConsumedDeviceSamples(t *testing.T) {
	...
	sink, err := devicert.NewRTCDeviceSink(registry, "")
	if err != nil {
	...
	if err := sink.WritePlaybackCue(context.Background(), cancelPlaybackFrame(101)); err != nil {
		t.Fatalf("queue pre-response hold-tone frame: %v", err)
	}

	response := audio.PlaybackResponse{ResponseID: "resp-device-clock", ItemID: "item-device-clock"}
	sink.StartPlayback(response)
	frames := make([]audio.PCMFrame, 10)
	for index := range frames {
		frames[index] = audio.PCMFrame{
...
func TestRTCDeviceSinkInterruptionTimerSurvives24kTo16kConversion(t *testing.T) {
	...
	sink, err := devicert.NewRTCDeviceSinkAtRate(registry, "", 24000)
	if err != nil {
	...
	response := audio.PlaybackResponse{ResponseID: "resp-mixed-rate", ItemID: "item-mixed-rate"}
...
func TestRTCDeviceSinkInterruptionUsesAudibleResponseAcrossQueuedContinuation(t *testing.T) {
	...
	holdToneConfig.GapThreshold = time.Hour
	sink.SetHoldToneConfig(holdToneConfig)

	media := audio.NewSessionMediaAtRate(nil, audio.SampleRate)
	defer func() { _ = media.Close() }()
	first := audio.PlaybackResponse{ResponseID: "resp-before-tool", ItemID: "item-before-tool"}
	second := audio.PlaybackResponse{ResponseID: "resp-after-tool", ItemID: "item-after-tool"}
...
func TestRTCDeviceSinkInterruptionExcludesPostResponseHoldToneSamples(t *testing.T) {
	for _, pulseCount := range []int{1, 5} {
		t.Run(fmt.Sprintf("%d hold-tone pulses", pulseCount), func(t *testing.T) {
			...
			response := audio.PlaybackResponse{ResponseID: "resp-hold-tone", ItemID: "item-hold-tone"}
			sink.StartPlayback(response)
			if err := sink.WritePlayback(context.Background(), cancelPlaybackFrame(401)); err != nil {
...
func TestRTCDeviceSinkCompletedResponseIgnoresLaterServerVADAfterHoldTones(t *testing.T) {
	for _, pulseCount := range []int{1, 5} {
		t.Run(fmt.Sprintf("%d hold-tone pulses", pulseCount), func(t *testing.T) {
			...
			response := audio.PlaybackResponse{ResponseID: "resp-complete", ItemID: "item-complete"}
			sink.StartPlayback(response)
			pumpErr := make(chan error, 1)
			go func() {
				pumpErr <- sink.Pump(context.Background(), &recordingRTCInboundMedia{frames: []audio.PCMFrame{{
...
func writeAndConsumeHoldTonePulse(t *testing.T, sink *devicert.RTCDeviceSink, registry *devicegw.SimulatedDuplexRegistry, value int16) {
	t.Helper()
	...
	for remaining > 0 {
		...
		if err := sink.WritePlaybackCue(context.Background(), cancelPlaybackSamples(samples, value)); err != nil {
...
func TestRTCDeviceBoundSessionDropsInFlightPlaybackAcrossCancelAndResume(t *testing.T) {
	...
	inbound := &cancelBoundaryInboundMedia{
		frame: audio.PCMFrame{Samples: stale},
		onFirstRead: func() {
	...
	if err := binding.Sink.Pump(context.Background(), &recordingRTCInboundMedia{
		frames: []audio.PCMFrame{{Samples: next}},
	}); err != nil {
...
func (m *cancelBoundaryInboundMedia) ReadFrame(context.Context) (audio.PCMFrame, error) {
	m.mu.Lock()
	if m.read {
		m.mu.Unlock()
