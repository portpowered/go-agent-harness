...
type RoomLaunchParticipantPlan struct {
	...
	InputDevice          devicegw.Device
	OutputDevice         devicegw.Device
	Provider             string
	Model                string
...
func ResolveBareRoomLaunchPlan(options RoomLaunchOptions) (RoomLaunchPlan, error) {
	...
	input, err := resolveBareRoomDevice(options.DeviceRegistry, devicegw.DirectionInput)
	if err != nil {
		return RoomLaunchPlan{}, err
	}
	output, err := resolveBareRoomDevice(options.DeviceRegistry, devicegw.DirectionOutput)
	if err != nil {
		return RoomLaunchPlan{}, err
	}
...
type roomLaunchParticipantDevices struct {
	input  devicegw.Device
	output devicegw.Device
}
...
func resolveConfiguredRoomDevices(manifest room.Manifest, registry devicegw.DeviceRegistry) (map[string]roomLaunchParticipantDevices, error) {
	hasHuman := false
	for _, participant := range manifest.Participants {
	...
	if registry == nil {
		return nil, configuredRoomDeviceValidationError(0, "input_device", "", "audio device registry is unavailable; run agent devices list", devicegw.ErrNilDeviceRegistry)
	}
	...
	byID := make(map[devicegw.DeviceID]devicegw.Device, len(devices))
	...
	for index, participant := range manifest.Participants {
		...
		input, err := resolveConfiguredRoomDevice(byID, index, "input_device", participant.InputDevice, devicegw.DirectionInput)
		if err != nil {
			return nil, err
		}
		output, err := resolveConfiguredRoomDevice(byID, index, "output_device", participant.OutputDevice, devicegw.DirectionOutput)
		if err != nil {
			return nil, err
		}
...
func resolveConfiguredRoomDevice(devices map[devicegw.DeviceID]devicegw.Device, participantIndex int, field, selector string, direction devicegw.Direction) (devicegw.Device, error) {
	device, ok := devices[selector]
	if !ok {
		return devicegw.Device{}, configuredRoomDeviceValidationError(participantIndex, field, selector, "device is unavailable; run agent devices list", devicegw.NewDeviceNotFoundError(selector))
	}
	if err := device.Validate(); err != nil {
		return devicegw.Device{}, configuredRoomDeviceValidationError(participantIndex, field, selector, err.Error(), err)
	}
	if device.Direction != direction {
		mismatch := &devicegw.DeviceDirectionError{
			ID:        device.ID,
			Direction: direction,
			Want:      direction,
			Got:       device.Direction,
			Kind:      devicegw.ErrDeviceDirectionMismatch,
		}
		return devicegw.Device{}, configuredRoomDeviceValidationError(participantIndex, field, selector, mismatch.Error(), mismatch)
	}
	return device, nil
}
...
func resolveBareRoomDevice(registry devicegw.DeviceRegistry, direction devicegw.Direction) (devicegw.Device, error) {
	device, err := registry.Default(direction)
	if err != nil {
		return devicegw.Device{}, &RoomLaunchDeviceError{Direction: direction, Err: err}
	}
	if err := device.Validate(); err != nil {
		return devicegw.Device{}, &RoomLaunchDeviceError{Direction: direction, Err: err}
	}
	if device.Direction != direction {
		return devicegw.Device{}, &RoomLaunchDeviceError{
			Direction: direction,
			Err: &devicegw.DeviceDirectionError{
				ID:        device.ID,
				Direction: direction,
				Want:      direction,
				Got:       device.Direction,
