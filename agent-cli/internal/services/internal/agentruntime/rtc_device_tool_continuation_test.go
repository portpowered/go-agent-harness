...
func assertToolContinuationScenario(t *testing.T, providerRate int, duplicateOpen bool, responseSizes []int) {
	...
	sink, err := devicert.NewRTCDeviceSinkAtRate(registry, "", providerRate)
	if err != nil {
	...
	media := audio.NewSessionMediaAtRate(nil, providerRate)
	defer func() { _ = media.Close() }()
	responses := make([]audio.PlaybackResponse, len(responseSizes))
	pcm := make([][]int16, len(responseSizes))
	want := make([]int16, 0)
	for index, size := range responseSizes {
		responses[index] = audio.PlaybackResponse{
		...
		reference, err := wavio.NewPCM16Resampler(providerRate, audio.SampleRate)
		if err != nil {
			t.Fatal(err)
		}
		converted, err := reference.Process(pcm[index], true)
		if err != nil {
	...
	if consumed := devicert.ConsumedPlaybackSamples(sink.PlaybackStats()); consumed != uint64(len(want)) {
		t.Fatalf("device consumed %d samples, want %d", consumed, len(want))
	}
	if got := trimToolContinuationEdgeUnderflow(registry.RenderedSamples()); !reflect.DeepEqual(got, trimToolContinuationEdgeUnderflow(want)) {
		t.Fatalf("tool continuation rendered %d/%d samples; response handoff lost %d samples", len(got), len(want), len(want)-len(got))
	}
}

func newToolContinuationDevice(t *testing.T, sampleRate int) *devicegw.SimulatedDuplexRegistry {
	t.Helper()
	registry, err := devicegw.NewSimulatedDuplexRegistry(devicegw.DuplexScenario{
		Seed:    2201,
		Render:  devicegw.ClockSpec{NominalRate: sampleRate, Quanta: []int{sampleRate * 30 / 1000}},
		Capture: devicegw.ClockSpec{NominalRate: sampleRate, Quanta: []int{sampleRate * 30 / 1000}},
	})
	if err != nil {
...
func waitForToolContinuationBacklog(t *testing.T, sink *devicert.RTCDeviceSink) {
	t.Helper()
...
func drainToolContinuationPlayback(t *testing.T, registry *devicegw.SimulatedDuplexRegistry, sink *devicert.RTCDeviceSink, pumpErr <-chan error) {
	t.Helper()
