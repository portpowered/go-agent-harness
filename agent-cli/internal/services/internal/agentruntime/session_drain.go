...
func waitForSessionLoopStragglers(out io.Writer, loop *agentloop.AgentLoop, policy sessionStragglerDrainPolicy, obs sessiontrace.Observer) error {
	return waitForSessionLoopStragglersWithContext(context.Background(), out, loop, policy, obs, nil, nil)
}
func waitForSessionLoopStragglersWithContext(ctx context.Context, out io.Writer, loop *agentloop.AgentLoop, policy sessionStragglerDrainPolicy, obs sessiontrace.Observer, source platformclock.Source, audioService audioio.Service) error {
	...
	// A deterministic timer is the canonical quiet-period clock, but it may
	// stop advancing as soon as the owning session enters teardown. Keep an
	// independent wall-time bound so a terminal path cannot strand resources
	// waiting for a virtual tick that no longer has an owner. This does not
	// replace the injected timer: whenever the virtual clock advances normally,
...
func shouldStopSessionLoop(msg messages.StreamMessage, opts sessionLoopOptions) bool {
	// Provider closure is authoritative even if it precedes SESSION.OPEN and
	// therefore prevents our requested close handshake from being sent.
