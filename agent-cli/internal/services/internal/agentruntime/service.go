...
func (d *Dispatcher) Run(ctx context.Context, out io.Writer, request public.Request) (runErr error) {
	...
	if err := validateSessionRunContext(ctx, out); err != nil {
		return err
	}
...
func validateSessionRunContext(ctx context.Context, out io.Writer) error {
...
func (d *Dispatcher) setupTrace(request public.Request, options *SessionRunOptions) (sessiontrace.Prepared, error) {
	return sessiontracewire.NewService().Prepare(sessiontrace.Request{
		...
		Device: sessiontrace.DeviceBinding{
			PreGateSamplesObserver:     sessiontrace.CaptureSamplesObserver(options.RTCDeviceBinding.PreGateSamplesObserver),
			UploadedSamplesObserver:    sessiontrace.CaptureSamplesObserver(options.RTCDeviceBinding.UploadedSamplesObserver),
			PlaybackSamplesObserver:    sessiontrace.PlaybackSamplesObserver(options.RTCDeviceBinding.PlaybackSamplesObserver),
			RenderedSamplesObserver:    sessiontrace.CaptureSamplesObserver(options.RTCDeviceBinding.RenderedSamplesObserver),
			RenderedSamplesUnavailable: options.RTCDeviceBinding.RenderedSamplesUnavailable,
		},
	})
}
