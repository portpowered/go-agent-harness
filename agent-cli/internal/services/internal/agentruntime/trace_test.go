...
func TestTraceStagesOutsideBundleThenAttaches(t *testing.T) {
	...
	options := newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), Clock: clock.NewDeterministic(time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC), time.Millisecond)})
...
func TestFailedSessionRetainsStagedTraceWithoutModifyingBundle(t *testing.T) {
	bundle := t.TempDir()
	request := public.Request{RecordDirectory: bundle}
	options := newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), Clock: clock.Real{}})
	trace, err := prepareTrace(&request, &options, options.Clock)
	if err != nil {
		t.Fatal(err)
	}
