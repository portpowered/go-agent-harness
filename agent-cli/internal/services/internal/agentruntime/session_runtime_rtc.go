...
func NewSessionRTCRuntimeFactoryWithObservability(components SessionRTCComponents, sampler observability.MetricSampler, logger observability.Logger) SessionRTCRuntimeFactory {
	return func(selection SessionRuntimeSelection) (SessionRTCRuntime, error) {
		if err := validateSessionRTCComponents(components); err != nil {
			return nil, err
		}
...
func validateSessionRTCComponents(c SessionRTCComponents) error {
...
func (r *sessionComposedRTCRuntime) Start(ctx context.Context) (SessionRTCDataPlane, error) {
	...
	var media sharedaudio.InboundMedia
...
func closeSessionRTCResources(media sharedaudio.InboundMedia, dataPlane SessionRTCDataPlane, signaling rtc.Signaling) error {
	return errors.Join(
