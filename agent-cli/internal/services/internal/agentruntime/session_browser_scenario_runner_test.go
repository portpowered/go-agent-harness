...
func TestRunBrowserConversationDefaultRunnerUsesSharedDuplexAudioPath(t *testing.T) {
	...
	result, err := RunBrowserConversation(context.Background(), nil, BrowserConversationRunOptions{
		...
		SessionOptions: newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), AudioService: newTestAudioIOService(), SessionInferencer: sessionInferencer}),
	})
	if err != nil {
		t.Fatalf("RunBrowserConversation: %v", err)
	}
...
func TestRunBrowserConversationInterruptsInFlightWorkAndPreservesDetachedTab(t *testing.T) {
	...
	result, err := RunBrowserConversation(context.Background(), nil, BrowserConversationRunOptions{
		...
		SessionOptions: newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), AudioService: newTestAudioIOService(), SessionInferencer: inferencer}),
	})
