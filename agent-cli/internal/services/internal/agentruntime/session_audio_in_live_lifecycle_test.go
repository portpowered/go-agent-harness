	...
	go func() {
		...
		for {
			...
			case turn := <-s.responses:
				...
				s.events <- `{"type":"response.output_audio.done"}`
				s.events <- `{"type":"response.done","response":{"id":"resp_` + string(rune('0'+turn)) + `","status":"completed"}}`
			case <-s.closed:
				return
			}
		}
		...
		case responseNumber := <-s.responses:
			if responseNumber == 2 {
				if !s.sendEvent(`{"type":"input_audio_buffer.speech_started"}`) ||
					...
					!s.sendEvent(`{"type":"response.done","response":{"id":"resp_tool","status":"completed"}}`) {
					return
				}
				continue
			}
			if !s.sendEvent(`{"type":"input_audio_buffer.speech_started"}`) ||
				...
				!s.sendEvent(`{"type":"response.done","response":{"id":"resp_`+string(rune('0'+responseNumber))+`","status":"completed"}}`) {
				return
			}
		case <-s.closed:
			return
...
type scheduledTurnDiagnosticSink struct {
	mu      sync.Mutex
	records []agentruntime.SessionDiagnosticRecord
}

func (s *scheduledTurnDiagnosticSink) RecordSessionDiagnostic(record agentruntime.SessionDiagnosticRecord) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.records = append(s.records, record)
}

func (s *scheduledTurnDiagnosticSink) recordsSnapshot() []agentruntime.SessionDiagnosticRecord {
	s.mu.Lock()
	defer s.mu.Unlock()
	return append([]agentruntime.SessionDiagnosticRecord(nil), s.records...)
}

func liveAudioInRunOptions(t *testing.T, dialer *audioInLifecycleServer, recordPath string) agentruntime.SessionRunOptions {
	t.Helper()
	return agentruntime.SessionRunOptions{
...
func TestLiveRecordRuntimeAudioInCompletesRoundTrip(t *testing.T) {
	...
	go func() {
		runErr <- agentruntime.RunSessionWithInstructionsAndAudioInputAndOutputAndTextSeedAndMaxDuration(
			context.Background(),
			os.Stdout,
			func() agentruntime.SessionRunOptions {
...
func TestLiveRecordRuntimeScheduledAudioContinuesAfterEmptyDirectoryResult(t *testing.T) {
	...
	var turnRecords []agentruntime.SessionDiagnosticRecord
	for _, record := range diagnostics.recordsSnapshot() {
		if record.Event == agentruntime.SessionDiagnosticEventTurn {
...
func TestLiveRecordRuntimeScheduledAudioBargeInWaitsForPromptResponse(t *testing.T) {
	...
	var terminalMetrics []agentruntime.SessionDiagnosticRecord
	for _, record := range diagnostics.recordsSnapshot() {
		if record.Event == agentruntime.SessionDiagnosticEventMetrics {
	...
	for field, want := range map[string]string{
		agentruntime.SessionDiagnosticFieldCompletedTurnCount:   "2",
		agentruntime.SessionDiagnosticFieldDispatchedInputCount: "2",
		agentruntime.SessionDiagnosticFieldScheduledInputCount:  "2",
	} {
...
func TestLiveRecordRuntimeScheduledAudioConfigTimeoutSendsNoTurn(t *testing.T) {
	...
	select {
	case err := <-result:
		if !errors.Is(err, agentruntime.ErrSessionScheduledAudioConfigTimeout) {
...
func TestLiveRecordRuntimeAudioInCancellationDuringAwaitSurfacesError(t *testing.T) {
	...
	result := make(chan error, 1)
	go func() {
		result <- agentruntime.RunSessionWithInstructionsAndAudioInputAndOutputAndTextSeedAndMaxDuration(
			ctx,
			os.Stdout,
...
func TestRunSessionWithAudioInputEndOfTurnLostSurfacesError(t *testing.T) {
	...
	result := make(chan error, 1)
	go func() {
		result <- agentruntime.RunSessionWithAudioInput(ctx, os.Stdout, agentruntime.SessionRunOptions{
