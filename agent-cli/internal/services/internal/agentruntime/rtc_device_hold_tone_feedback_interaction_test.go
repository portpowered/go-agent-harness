...
func TestRTCDeviceSinkHoldToneConfirmedFeedbackStillForwardsGenuineBargeInPromptly(t *testing.T) {
	...
	binding.Sink.SetHoldToneConfig(audio.HoldToneConfig{
	...
	binding.Sink.SetHoldToneTick(5 * time.Millisecond)

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	inbound := newFeedbackInbound(2)
	outbound := &feedbackOutbound{frames: make(chan audio.PCMFrame, 256)}
