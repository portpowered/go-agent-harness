...
func newRoomHumanTestRegistry(t *testing.T) *roomHumanTestRegistry {
	t.Helper()
	inputDevice, err := devicegw.NewDevice("test", "microphone", "Test Microphone", devicegw.DirectionInput)
	if err != nil {
		t.Fatalf("new input device: %v", err)
	}
	outputDevice, err := devicegw.NewDevice("test", "speakers", "Test Speakers", devicegw.DirectionOutput)
	if err != nil {
...
func (r *roomHumanTestRegistry) List() ([]devicegw.Device, error) {
	return []devicegw.Device{r.inputDevice, r.outputDevice}, nil
}

func (r *roomHumanTestRegistry) Default(direction devicegw.Direction) (devicegw.Device, error) {
	switch direction {
	case devicegw.DirectionInput:
		return r.inputDevice, nil
	case devicegw.DirectionOutput:
...
func (r *roomHumanTestRegistry) Open(id devicegw.DeviceID) (devicegw.OpenedDevice, error) {
	switch id {
...
var _ devicegw.DeviceRegistry = (*roomHumanTestRegistry)(nil)
...
func (h *roomHumanInputHandle) DeviceDirection() devicegw.Direction { return devicegw.DirectionInput }
...
func (h *roomHumanOutputHandle) DeviceDirection() devicegw.Direction { return devicegw.DirectionOutput }
