...
func (w *webMCPRealtimeWire) sendInitialListTurn() {
	...
	w.enqueue(map[string]any{"type": "response.done", "response": map[string]any{"id": "response-" + webMCPRealtimeListCallID}})
}
...
func (w *webMCPRealtimeWire) acceptContinuation() {
	...
	if listSeen && !w.invokeTurnSent {
		...
		w.enqueue(map[string]any{"type": "response.done", "response": map[string]any{"id": "response-" + webMCPRealtimeInvokeCallTwo}})
		return
	}
	if invokeCount == 2 && !w.finalTurnSent {
		...
		w.enqueue(map[string]any{"type": "response.done", "response": map[string]any{"id": "response-final"}})
