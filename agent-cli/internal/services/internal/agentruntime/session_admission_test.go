...
func TestValidateSessionRunOptionsAdmitsSessionsWithoutCaptureConfiguration(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			opts: newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), Prompt: "hello", PromptProvided: true}),
		},
		{
			name: "scheduled audio input, no record or replay",
			opts: newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), AudioInputs: []ScheduledAudioInput{{}}}),
		},
		{
			name: "bare invocation, no record or replay",
			opts: newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), BareLive: true}),
		},
		{
			name: "browser tools enabled, no record or replay",
			opts: newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), BrowserToolsEnabled: true}),
		},
		{
...
func TestValidateSessionRunOptionsStillRejectsRecordAndReplayTogether(t *testing.T) {
	err := validateSessionRunOptions(newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), RecordPath: "a.json", ReplayPath: "b.json"}))
...
func TestValidateSessionRunOptionsStillRejectsNonJSONRecordPath(t *testing.T) {
	err := validateSessionRunOptions(newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), RecordPath: "capture.txt"}))
...
func TestValidateSessionRunOptionsAdmitsRecordAndReplay(t *testing.T) {
	if err := validateSessionRunOptions(newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), RecordPath: filepath.Join(t.TempDir(), "capture.json")})); err != nil {
	...
	if err := validateSessionRunOptions(newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), ReplayPath: replayPath})); err != nil {
		t.Fatalf("validateSessionRunOptions(--replay) = %v, want admission", err)
	}
}

func TestValidateSessionRunOptionsReplayTiming(t *testing.T) {
	if err := validateSessionRunOptions(newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), ReplayTiming: "recorded"})); err == nil || !strings.Contains(err.Error(), "requires --replay") {
		t.Fatalf("recorded timing without replay error = %v, want --replay requirement", err)
	}
	if err := validateSessionRunOptions(newTestSessionRunOptions(SessionRunOptions{ModelCatalog: testModelCatalog(), ReplayTiming: "elastic"})); err == nil || !strings.Contains(err.Error(), "immediate or recorded") {
