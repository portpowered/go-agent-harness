...
func TestSessionStragglerDrainFrozenClockHasWallSafety(t *testing.T) {
	...
	start := time.Now()
	if err := waitForSessionLoopStragglersWithContext(context.Background(), io.Discard, loop, defaultSessionStragglerDrainPolicy, nil, clock, audioiowire.NewService()); err != nil {
...
func TestSessionStragglerDrainCancellationWinsImmediately(t *testing.T) {
	...
	cancel()
	start := time.Now()
	if err := waitForSessionLoopStragglersWithContext(ctx, io.Discard, loop, defaultSessionStragglerDrainPolicy, nil, clock, audioiowire.NewService()); err != nil {
