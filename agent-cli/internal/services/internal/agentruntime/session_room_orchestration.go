...
func prepareRoomReplayOptions(opts RoomRunOptions, validation room.ValidationOptions) (RoomRunOptions, room.ValidationOptions, bool, error) {
	var replayPlan roomreplay.RoomReplayPlan
	var replayMode bool
	var err error
	if opts.ReplayPlan != nil {
		if opts.ReplayService == nil {
			return opts, validation, true, errors.New("room replay service is required")
		}
		replayPlan, err = opts.ReplayService.Load(opts.ReplayPlan.BundlePath)
		replayMode = true
	} else if path := strings.TrimSpace(opts.ReplayPath); path != "" {
		if opts.ReplayService == nil {
			return opts, validation, true, errors.New("room replay service is required for replay path admission")
		}
		replayPlan, err = opts.ReplayService.Load(path)
		replayMode = true
	}
