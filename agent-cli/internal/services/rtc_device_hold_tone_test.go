...
func newStepRTCInboundMedia() *stepRTCInboundMedia {
	return &stepRTCInboundMedia{frames: make(chan rtc.PCMFrame, 4)}
}

func (m *stepRTCInboundMedia) ReadFrame(ctx context.Context) (rtc.PCMFrame, error) {
	select {
...
func readUntilSignalOrDeadline(t *testing.T, source *audio.DeviceSource, deadline time.Time) bool {
	t.Helper()
...
func TestRTCDeviceSinkHoldToneFillsGapLongerThanThreshold(t *testing.T) {
	...
	sink.holdToneConfig = testHoldToneSinkConfig()
	sink.holdToneTick = 5 * time.Millisecond
	...
	inbound.frames <- rtc.PCMFrame{Samples: first}
...
func TestRTCDeviceSinkHoldToneRealAudioReachesDeviceUnmodifiedAfterGap(t *testing.T) {
	...
	inbound.frames <- rtc.PCMFrame{Samples: second}
	inbound.frames <- rtc.PCMFrame{Samples: padding}
