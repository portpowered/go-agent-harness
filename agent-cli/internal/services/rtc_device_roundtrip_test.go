...
const (
	rtcRoundtripInputID    audio.DeviceID = "virtual:mic-in"
	rtcRoundtripMicFeedID  audio.DeviceID = "virtual:mic-feed"
	rtcRoundtripOutputID   audio.DeviceID = "virtual:speaker-out"
	rtcRoundtripSpeakerID  audio.DeviceID = "virtual:speaker-observe"
...
func TestRTCDeviceBindingVirtualRegistryTrackRoundTrip(t *testing.T) {
	...
	assertRTCDeviceRoundtripShutdown(t, "source", sourceDone, func(err error) bool {
		return errors.Is(err, context.Canceled) || errors.Is(err, services.ErrRTCDeviceSourceClosed)
	})
	assertRTCDeviceRoundtripShutdown(t, "sink", sinkDone, func(err error) bool {
		return errors.Is(err, context.Canceled) || errors.Is(err, services.ErrRTCDeviceSinkClosed)
	})
...
func newRTCDeviceRoundtripRegistry(t *testing.T) *audio.VirtualRegistry {
	t.Helper()
	registry, err := audio.NewVirtualRegistry(audio.VirtualBackendConfig{
		Devices: []audio.VirtualDeviceConfig{
			{ID: "mic-in", Name: "Virtual Microphone", Direction: audio.DirectionInput, LoopbackID: "mic-feed"},
			{ID: "mic-feed", Name: "Virtual Microphone Feeder", Direction: audio.DirectionOutput, LoopbackID: "mic-in"},
			{ID: "speaker-out", Name: "Virtual Speaker", Direction: audio.DirectionOutput, LoopbackID: "speaker-observe"},
			{ID: "speaker-observe", Name: "Virtual Speaker Observer", Direction: audio.DirectionInput, LoopbackID: "speaker-out"},
		},
		Defaults: map[audio.Direction]string{
			audio.DirectionInput:  "mic-in",
			audio.DirectionOutput: "speaker-out",
		},
	})
	if err != nil {
...
func assertRTCDeviceRoundtripIDs(t *testing.T, registry *audio.VirtualRegistry) {
	t.Helper()
	...
	for _, device := range devices {
		backend, nativeID, err := audio.ParseDeviceID(device.ID)
		if err != nil {
			t.Fatalf("parse device ID %q: %v", device.ID, err)
		}
		if backend != audio.VirtualBackendName || nativeID == "" {
...
func newLoopbackRTCTrackPeer(frameCount int) *loopbackRTCTrackPeer {
	return &loopbackRTCTrackPeer{
		frames: make(chan rtc.PCMFrame, frameCount),
		closed: make(chan struct{}),
	}
}

func (p *loopbackRTCTrackPeer) WriteFrame(ctx context.Context, frame rtc.PCMFrame) error {
	if ctx == nil {
	...
	owned := rtc.PCMFrame{Samples: append([]int16(nil), frame.Samples...)}
	select {
	case p.frames <- owned:
		p.mu.Lock()
		p.writes++
...
func (p *loopbackRTCTrackPeer) ReadFrame(ctx context.Context) (rtc.PCMFrame, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	select {
	case frame := <-p.frames:
		if len(frame.Samples) != audio.FrameSize {
			return rtc.PCMFrame{}, fmt.Errorf("loopback inbound frame has %d samples, want %d", len(frame.Samples), audio.FrameSize)
		}
		p.mu.Lock()
		p.reads++
		p.mu.Unlock()
		return rtc.PCMFrame{Samples: append([]int16(nil), frame.Samples...)}, nil
	case <-p.closed:
...
var _ rtc.InboundMedia = (*loopbackRTCTrackPeer)(nil)
var _ rtc.OutboundMedia = (*loopbackRTCTrackPeer)(nil)
