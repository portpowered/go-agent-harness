...
func newSelfPlayEvidence(destination string, opts SelfPlayRunOptions, startedAt time.Time) (*selfPlayEvidence, error) {
	...
	for index, config := range configs {
		...
		side.runtimeRecord = newSessionRuntimeObservationRecorder(side.runtime, nil)
		evidence.sides[index] = side
	}
	return evidence, nil
}
...
func newSelfPlayWAVRecorder(path string, sampleRate int) (*selfPlayWAVRecorder, error) {
	...
	header, err := selfPlayWAVHeader(sampleRate, 0)
	if err == nil {
...
func (w *selfPlayWAVRecorder) close() error {
	...
	if header, headerErr := selfPlayWAVHeader(w.sampleRate, w.dataBytes); headerErr != nil {
...
func selfPlayWAVHeader(sampleRate int, dataBytes uint64) ([selfPlayWAVHeaderSize]byte, error) {
	var header [selfPlayWAVHeaderSize]byte
	if sampleRate <= 0 {
		return header, fmt.Errorf("WAV sample rate must be positive, got %d", sampleRate)
	}
	maximumDataBytes := uint64(^uint32(0)) - 36
	if dataBytes > maximumDataBytes {
		return header, fmt.Errorf("WAV data size %d exceeds the 32-bit RIFF limit", dataBytes)
	}
	if dataBytes%2 != 0 {
		return header, fmt.Errorf("WAV PCM16 data size %d is not sample aligned", dataBytes)
	}
	binary.LittleEndian.PutUint32(header[0:4], uint32('R')|uint32('I')<<8|uint32('F')<<16|uint32('F')<<24)
	binary.LittleEndian.PutUint32(header[4:8], uint32(36+dataBytes))
	binary.LittleEndian.PutUint32(header[8:12], uint32('W')|uint32('A')<<8|uint32('V')<<16|uint32('E')<<24)
	binary.LittleEndian.PutUint32(header[12:16], uint32('f')|uint32('m')<<8|uint32('t')<<16|uint32(' ')<<24)
	...
	binary.LittleEndian.PutUint32(header[36:40], uint32('d')|uint32('a')<<8|uint32('t')<<16|uint32('a')<<24)
