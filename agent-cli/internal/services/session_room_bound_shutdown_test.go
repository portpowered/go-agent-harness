...
func TestRunRoom_MaxTurnsDrainsResponseAlreadyInFlight(t *testing.T) {
	...
	if active.TerminationTrigger != ParticipantTerminationTriggerMaxTurnsReachedMidResponse || active.TerminationDisposition != ParticipantTerminationDispositionCompletedDuringGrace || active.Classification != "" || active.TerminalReason != messages.TerminalReasonProviderAuthoredCompletion || active.TerminalProvenance != messages.TerminalProvenanceProvider || active.OutputState != messages.TerminalOutputComplete {
		t.Fatalf("active bound completion metadata = %+v", active)
	}
	peer := result.Participants[peerID]
	if peer.TerminationTrigger != ParticipantTerminationTriggerMaxTurnsReached || peer.TerminationDisposition != ParticipantTerminationDispositionCompleted || peer.TerminalReason != messages.TerminalReasonProviderAuthoredCompletion || peer.TerminalProvenance != messages.TerminalProvenanceProvider || peer.OutputState != messages.TerminalOutputComplete {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			for _, id := range []string{activeID, peerID} {
				...
				if id == activeID {
					...
					if participant.TerminationTrigger != wantTrigger || participant.TerminationDisposition != ParticipantTerminationDispositionCancelledAfterGrace || participant.Classification != RoomBoundCancelledClassification || participant.TerminalReason != messages.TerminalReasonCancellation || participant.TerminalProvenance != messages.TerminalProvenanceRoom {
						t.Fatalf("participant %q cancellation metadata = %+v", id, participant)
					}
					if participant.OutputState != messages.TerminalOutputNone && participant.OutputState != messages.TerminalOutputPartial {
