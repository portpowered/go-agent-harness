...
func TestRTCDeviceSinkCapturedFirstTurnPreservesPacedVirtualPlayback(t *testing.T) {
	...
	expectedDeviceSamples, err := wavio.Resample(providerSamples, wavio.Rate24kHz, audio.SampleRate)
	if err != nil {
...
func capturedFirstTurnFrames(t *testing.T, samples []int16) []rtc.PCMFrame {
	t.Helper()
