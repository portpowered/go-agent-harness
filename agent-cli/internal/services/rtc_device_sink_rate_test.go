...
func TestRTCDeviceSinkConvertsProviderRateToSupportedPlaybackRate(t *testing.T) {
	...
	observe, err := audio.NewDeviceSourceAtRate(registry, "virtual:input", audio.SampleRate)
	if err != nil {
	...
	inbound := &recordingRTCInboundMedia{frames: []rtc.PCMFrame{{Samples: providerSamples}}}
...
func assertShort24kSessionMediaPlayback(t *testing.T, responseDone bool) {
	...
	media := rtc.NewSessionMediaAtRate(nil, wavio.Rate24kHz)
...
func TestRTCDeviceSinkKeepsMatchedPlaybackRateIdentity(t *testing.T) {
	...
	observe, err := audio.NewDeviceSourceAtRate(registry, "virtual:input", wavio.Rate24kHz)
	if err != nil {
		t.Fatalf("open playback observer: %v", err)
	}
	defer func() { _ = observe.Close() }()
	...
	if err := sink.Pump(context.Background(), &recordingRTCInboundMedia{frames: []rtc.PCMFrame{{Samples: want}}}); err != nil {
		t.Fatalf("Pump: %v", err)
	}
	got := make([]int16, audio.FrameSize)
	readCtx, cancel := context.WithTimeout(context.Background(), time.Second)
...
func newRTCDeviceSinkRateRegistry(t *testing.T, rate int) *audio.VirtualRegistry {
	t.Helper()
