...
func TestRTCDeviceAudioCaptureReplaysThroughDevices(t *testing.T) {
	...
	liveInferencer, err := services.NewGrokSessionInferencerWithOptions(
	...
	replayInferencer, err := services.NewGrokSessionInferencerWithOptions(
...
func newRecordedRTCDeviceRoundtripRegistry(t *testing.T) *audio.VirtualRegistry {
	t.Helper()
	registry, err := audio.NewVirtualRegistry(audio.VirtualBackendConfig{
		RecordPCM: true,
		Devices: []audio.VirtualDeviceConfig{
			{ID: "mic-in", Name: "Recorded Microphone", Direction: audio.DirectionInput, LoopbackID: "mic-feed"},
			{ID: "mic-feed", Name: "Recorded Microphone Feed", Direction: audio.DirectionOutput, LoopbackID: "mic-in"},
			{ID: "speaker-out", Name: "Recorded Speaker", Direction: audio.DirectionOutput, LoopbackID: "speaker-observe"},
			{ID: "speaker-observe", Name: "Recorded Speaker Observer", Direction: audio.DirectionInput, LoopbackID: "speaker-out"},
		},
		Defaults: map[audio.Direction]string{audio.DirectionInput: "mic-in", audio.DirectionOutput: "speaker-out"},
	})
	if err != nil {
...
func exerciseRTCDeviceAudioPass(t *testing.T, registry *audio.VirtualRegistry, inferencer messages.SessionInferencer, afterOutput func()) {
	t.Helper()
