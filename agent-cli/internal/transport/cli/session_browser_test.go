...
`
	...
	deviceRegistry, err := devicegw.NewVirtualRegistry(devicegw.DefaultVirtualBackendConfig())
	if err != nil {
...
func TestSessionManagedBrowserStartupFailureStopsBeforeProvider(t *testing.T) {
	// Satisfy credential preflight so this test isolates browser readiness.
	t.Setenv("AGENT_MODEL__OPENAI__API_KEY", "test-browser-startup-key")
	globalFlags := flags.NewGlobalFlags()
	globalFlags.ConfigDirPath = t.TempDir()
	inferencer := newBrowserAdmissionInferencer()
	defer inferencer.Close()
...
func (s *browserAdmissionSession) RTCMedia() sharedaudio.MediaEndpoints {
...
var _ sharedaudio.InboundMedia = (*browserAdmissionMedia)(nil)
var _ sharedaudio.OutboundMedia = (*browserAdmissionMedia)(nil)
