...
func TestRouterPreRunNormalizesSelfPlayOutputDirectory(t *testing.T) {
	...
	owner.SetRunner(func(_ context.Context, _ io.Writer, options runtimeSelfPlay.Request) error {
		got = options
		return nil
	})
...
func TestRouterPreRunNormalizesRoomManifestAndOutputPaths(t *testing.T) {
  ...
  "participants": [
	...
	globalFlags := flags.NewGlobalFlags()
	owner := newTestRoomRunCommand(globalFlags, nil)
	var got rooms.RoomRunOptions
	owner.SetRunner(func(_ context.Context, _ io.Writer, options rooms.RoomRunOptions) (rooms.RoomResult, error) {
		got = options
