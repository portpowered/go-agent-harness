...
func replayAudioAppendRecord(template gatewaytesting.CapturedSessionEvent, pcm []byte) (gatewaytesting.CapturedSessionEvent, error) {
	...
	}{Type: "input_audio_buffer.append", Audio: codec.EncodeBase64(pcm)})
	if err != nil {
...
func replayExecFunc(fixtures map[string]string, collectors ...serviceprobes.MetricsCollector) probe.ExecFunc {
	return func(ctx context.Context, scenario probe.Scenario) (probe.ObservationSnapshot, error) {
		...
		return observationFromSessionCapture(ctx, scenario, replayCapture, fixture, !injected, collectors...)
	}
}
