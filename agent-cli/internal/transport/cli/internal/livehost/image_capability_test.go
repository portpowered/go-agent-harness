...
func TestBuildRequestAdmitsSessionImagesOnlyForImageCapableModels(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			opened := 0
			open := func(paths []string) ([]messages.ContentPart, error) {
				opened++
			...
			// A session without --image still binds read_image; the tool path
			// must enforce the same capability decision.
			if _, err := BuildRequest(context.Background(), request, nil, deps); err != nil {
				t.Fatalf("BuildRequest without images: %v", err)
			}
			if toolOpen == nil {
				t.Fatal("read_image preparer was not bound")
			}
