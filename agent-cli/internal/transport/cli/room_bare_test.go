...
func TestRoomRunCommandBareInvocationPassesResolvedPlanToRunner(t *testing.T) {
	...
	command := newTestRoomRunCommand(globalFlags, registry)
...
func TestRoomRunCommandConfigAliasRemainsAuthoritative(t *testing.T) {
	...
	if got.LaunchPlan == nil || got.LaunchPlan.Mode != rooms.RoomLaunchModeConfigured || got.LaunchPlan.ConfigPath != manifestPath {
...
`)
	...
	if got.LaunchPlan == nil || got.LaunchPlan.Mode != rooms.RoomLaunchModeConfigured || got.LaunchPlan.ConfigPath != configPath {
...
func (r *bareRoomCLIRegistry) List() ([]devicegw.Device, error) {
	return []devicegw.Device{r.input, r.output}, nil
}

func (r *bareRoomCLIRegistry) Default(direction devicegw.Direction) (devicegw.Device, error) {
	r.defaultCalls++
	switch direction {
...
func (r *bareRoomCLIRegistry) Open(id devicegw.DeviceID) (devicegw.OpenedDevice, error) {
	r.openCalls++
