...
type Router struct {
	...
	SessionReplayCommand *SessionReplayCommand
...
// NewRouter constructs the command graph from its injected dependencies.
func NewRouter(
	flags *flags.GlobalFlags,
	rootCommand *RootCommand,
	askCommand *AskCommand,
	...
	if sessionCommand != nil {
		...
		customerSimulationCommand.ReplayService = sessionCommand.liveReplayService
	}
	return &Router{
		Flags:                     flags,
...
func (r *Router) BuildRoot() *cobra.Command {
	...
	sessionGroup.AddCommand(NewPath("replay <bundle-directory>", r.SessionReplayCommand.Generate()))
