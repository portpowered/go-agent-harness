...
func observationFromSessionCapture(ctx context.Context, scenario probe.Scenario, capture gatewaytesting.SessionCapture, sourcePath string, validateSource bool, collectors ...serviceprobes.MetricsCollector) (probe.ObservationSnapshot, error) {
	var (
	...
	if scenarioDeclaresMetricsReconciliation(scenario) {
		...
		metricsSeries, metricsErr := collectReplayMetricsEvidence(ctx, collector, sourcePath, scenarioSendText(scenario))
		if metricsErr != nil {
