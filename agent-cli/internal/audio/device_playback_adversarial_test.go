...
func TestVirtualPlaybackCapacityAdversarial(t *testing.T) {
	t.Run("01 zero request is a no-op", func(t *testing.T) {
		_, output, _ := adversarialVirtualPair(t, SampleRate)
	...
	t.Run("03 nil stream request is a no-op", func(t *testing.T) {
		...
		if err := output.WaitForPlaybackCapacity(context.Background(), FrameSize); err != nil {
	...
	t.Run("04 oversized request is rejected", func(t *testing.T) {
		_, output, _ := adversarialVirtualPair(t, SampleRate)
		_, high, err := PlaybackQueueWatermarks(output.DeviceFormat())
		if err != nil {
			t.Fatal(err)
		}
		if err := output.WaitForPlaybackCapacity(context.Background(), high+1); !errors.Is(err, ErrInvalidPlaybackQueue) {
	...
	t.Run("06 exact high watermark is admitted", func(t *testing.T) {
		...
		primeVirtualPlayback(t, output, high-FrameSize)
		if err := output.WaitForPlaybackCapacity(context.Background(), FrameSize); err != nil {
	...
	t.Run("08 waiter remains blocked above low watermark", func(t *testing.T) {
		_, output, input := adversarialVirtualPair(t, SampleRate)
		low, high, err := PlaybackQueueWatermarks(output.DeviceFormat())
		if err != nil {
			t.Fatal(err)
		}
		primeVirtualPlayback(t, output, high)
		wait := startCapacityWait(output, context.Background(), FrameSize)
		for output.PlaybackStats().QueuedSamples-FrameSize > low {
			if err := input.ReadSamples(context.Background(), make([]int16, FrameSize)); err != nil {
				t.Fatal(err)
			}
			assertCapacityWaitBlocked(t, wait)
		}
	...
	t.Run("09 context cancellation wakes waiter", func(t *testing.T) {
		_, output, _ := adversarialVirtualPair(t, SampleRate)
		_, high, _ := PlaybackQueueWatermarks(output.DeviceFormat())
		primeVirtualPlayback(t, output, high)
		ctx, cancel := context.WithCancel(context.Background())
		wait := startCapacityWait(output, ctx, FrameSize)
		cancel()
		if err := awaitCapacityWait(t, wait); !errors.Is(err, context.Canceled) {
	...
	t.Run("10 deadline wakes waiter", func(t *testing.T) {
		...
		defer cancel()
		if err := output.WaitForPlaybackCapacity(ctx, FrameSize); !errors.Is(err, context.DeadlineExceeded) {
	...
	t.Run("14 output removal wakes waiter", func(t *testing.T) {
		registry, output, _ := adversarialVirtualPair(t, SampleRate)
		_, high, _ := PlaybackQueueWatermarks(output.DeviceFormat())
		primeVirtualPlayback(t, output, high)
		wait := startCapacityWait(output, context.Background(), FrameSize)
	...
	t.Run("17 cancelled exact read returns context error", func(t *testing.T) {
		_, _, input := adversarialVirtualPair(t, SampleRate)
		ctx, cancel := context.WithCancel(context.Background())
		cancel()
	...
	t.Run("18 sustained paced FIFO has no drops", func(t *testing.T) {
		...
		for frameIndex := 0; frameIndex < frames; frameIndex++ {
			...
			if got[0] != int16(frameIndex*FrameSize) || got[len(got)-1] != int16(frameIndex*FrameSize+FrameSize-1) {
	...
	t.Run("19 48 kHz watermarks scale by duration", func(t *testing.T) {
		low, high, err := PlaybackQueueWatermarks(PCM16DeviceFormat(48000))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("20 malformed format rejects watermarks", func(t *testing.T) {
		if _, _, err := PlaybackQueueWatermarks(DeviceFormat{}); !errors.Is(err, ErrInvalidPlaybackQueue) {
...
func adversarialVirtualPair(t *testing.T, rate int) (*VirtualRegistry, *VirtualStream, *VirtualStream) {
	t.Helper()
	capability := VirtualCapability{SampleRate: rate, Channels: Channels, BitDepth: DeviceBitDepthPCM16, Format: DeviceEncodingPCM16}
	...
	openedOutput, err := registry.OpenWithFormat("virtual:output", PCM16DeviceFormat(rate))
	if err != nil {
		t.Fatal(err)
	}
	openedInput, err := registry.OpenWithFormat("virtual:input", PCM16DeviceFormat(rate))
	if err != nil {
