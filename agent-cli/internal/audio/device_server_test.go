...
func TestRemoteDeviceServerRoundTripUsesExplicitCallbackClock(t *testing.T) {
	...
	registry, err := audio.NewSimulatedDuplexRegistry(audio.DuplexScenario{
		Seed:    37,
		Render:  audio.ClockSpec{NominalRate: 16000, Quanta: []int{audio.FrameSize}},
		Capture: audio.ClockSpec{NominalRate: 16000, Quanta: []int{audio.FrameSize}},
	})
	if err != nil {
	...
	remote, err := audio.NewRemoteDeviceRegistry(endpoint)
	if err != nil {
	...
	input, err := remote.Default(audio.DirectionInput)
	if err != nil {
		t.Fatalf("remote input default: %v", err)
	}
	output, err := remote.Default(audio.DirectionOutput)
	if err != nil {
		t.Fatalf("remote output default: %v", err)
	}
	source, err := audio.NewDeviceSourceAtRate(remote, input.ID, 16000)
	if err != nil {
		t.Fatalf("open remote source: %v", err)
	}
	defer func() { _ = source.Close() }()
	sink, err := audio.NewDeviceSinkAtRate(remote, output.ID, 16000)
	if err != nil {
	...
	if err := audio.InjectRemoteDeviceServerCapture(context.Background(), endpoint, nearEnd); err != nil {
		t.Fatalf("inject remote microphone PCM: %v", err)
	}
	if err := audio.AdvanceRemoteDeviceServer(context.Background(), endpoint, 1); err != nil {
	...
	snapshot, err := audio.ReadRemoteDeviceServerSnapshot(context.Background(), endpoint)
	if err != nil {
	...
	if _, err := remote.Default(audio.Direction("sideways")); err == nil {
...
func TestRemoteDeviceServerBackpressureUnblocksOnAdvanceAndDiscard(t *testing.T) {
	...
	sink, err := audio.NewDeviceSinkAtRate(remote, "simulated-duplex:output", 16000)
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = sink.Close() }()
	...
	if err := audio.AdvanceRemoteDeviceServer(context.Background(), endpoint, 3); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-waiting:
...
func TestRemoteDeviceServerRejectsNonLoopbackAndMalformedEndpoints(t *testing.T) {
	for _, endpoint := range []string{"", "http://127.0.0.1:1234", "0.0.0.0:1234", "192.0.2.4:1234", "localhost", "localhost:bad port"} {
		if _, err := audio.NewRemoteDeviceRegistry(endpoint); !errors.Is(err, audio.ErrRemoteDeviceServerEndpoint) {
...
func TestRemoteDeviceServerPreservesTypedFormatErrors(t *testing.T) {
	...
	httpServer := httptest.NewServer(server.Handler())
	defer httpServer.Close()
	remote, err := audio.NewRemoteDeviceRegistry(strings.TrimPrefix(httpServer.URL, "http://"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemoteDeviceHandleReportsServerSideClose(t *testing.T) {
	...
	source, err := audio.NewDeviceSourceAtRate(remote, "simulated-duplex:input", 16000)
	if err != nil {
		t.Fatal(err)
	}
