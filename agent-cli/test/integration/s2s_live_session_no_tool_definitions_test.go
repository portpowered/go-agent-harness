...
func writeStrictOpenAIExecRoundTripCapture(t *testing.T, path, execCommand string) {
	...
	data, err := json.MarshalIndent(gwtesting.SessionCapture{
		...
		Records: []gwtesting.CapturedSessionEvent{
			...
			strictOpenAIWebSocketRecord(10, gwtesting.DirectionClientToServer, "conversation.item.create", `{"type":"conversation.item.create","item":{"type":"message","role":"user","content":[{"type":"input_text","text":"probe PROBE_TOOL_MARKER_9182"}]}}`),
			strictOpenAIWebSocketRecord(11, gwtesting.DirectionClientToServer, "response.create", `{"type":"response.create"}`),
